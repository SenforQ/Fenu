...
/*: "%@ reply %@：%@" :*/
fileprivate let app_oneStr:String = "%@ rboth initial birth ego"
fileprivate let mSunlightValue:String = "dirt：%dirt"
...
struct MonitorTransformable: HandyJSON {
    //: var cid: String?
...
extension MonitorTransformable {
    //: mutating func caculateReplyItemHeight() {
    mutating func relationHeight() {
        //: var tempContent = ""
        var tempContent = ""
        //: if replyUser.count>0 {
        if replyUser.count > 0 {
            //: tempContent = String(format: "%@ reply %@：%@", nickname, replyUser, content)
            tempContent = String(format: (String(app_oneStr.prefix(4)) + "eply %" + mSunlightValue.replacingOccurrences(of: "dirt", with: "@")), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(OrdinalThen.fontTotal(text: tempContent, size: CGSize(width: noti_senseContent - 82, height: CGFloat(MAXFLOAT)), font: UIFont.afterDisable(type: .Regular, fontSize: 14), lineNumber: 0).height)
