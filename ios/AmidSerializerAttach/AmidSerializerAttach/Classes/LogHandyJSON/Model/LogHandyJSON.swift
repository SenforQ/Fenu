...
struct ExactlyHandyJSON: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
struct BenchTransformable: HandyJSON {
    ...
    var status: UglySequence?
}

///
//: enum MomentVideoStatus: String {
enum UglySequence: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension LogHandyJSON {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func hemCustomer(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(OrdinalThen.fontTotal(text: content!, size: CGSize(width: noti_senseContent - 54, height: CGFloat(MAXFLOAT)), font: UIFont.afterDisable(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == PoorMirrorPath.Photo.rawValue || momentType == PoorMirrorPath.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
