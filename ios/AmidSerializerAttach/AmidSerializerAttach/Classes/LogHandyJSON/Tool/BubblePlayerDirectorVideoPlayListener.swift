...
/*: "/TXCache" :*/
fileprivate let dream_revenueEditionPressureValue:String = "/TXCreplacement quickly continue concept"
...
// __CLOSE_PRINT__
//
//  BubblePlayerDirectorVideoPlayListener.swift
//  AbroadTalking
//
...
protocol PosterPlayerDelegate: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func scope(player: BubblePlayerDirectorVideoPlayListener, status: InPublicPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func durationTime(player: BubblePlayerDirectorVideoPlayListener, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func readyProgress(player: BubblePlayerDirectorVideoPlayListener, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class BubblePlayerDirectorVideoPlayListener: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: PosterPlayerDelegate?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: InPublicPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = BubblePlayerDirectorVideoPlayListener()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(dream_revenueEditionPressureValue.prefix(4)) + "ache")) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(runBases), userInfo: nil, repeats: true)
...
extension BubblePlayerDirectorVideoPlayListener {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func agreeSmart(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: kKeyInsertValue.generalGame(), with: dreamVoiceMarginStr)
    ...
    @objc func runBases() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.readyProgress(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func am(renderMode: RaceRenderMode) {
    ...
    func fragmentTime(time: Float) {
        //: self.player.seek(time)
        self.player.seek(time)
    }

    //: func height() ->Int {
    func separateOutHeight() -> Int {
        //: return Int(self.player.height())
        return Int(self.player.height())
    }

    //: func width() ->Int {
    func questionableExist() -> Int {
    ...
    func offdForDoing(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isFind == true {
        if AdviceLandscapeReactiveCompatible.shared.isFind == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func dayShow() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.scope(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func creationChecked() -> Bool {
...
extension BubblePlayerDirectorVideoPlayListener: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.durationTime(player: self, duration: Int(duration), currentTime: Int(currentTime))
