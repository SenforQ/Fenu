...
fileprivate let notiToolMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Save successfully" :*/
fileprivate let kPrivacyValue:String = "man yesterday revenueSave s"
fileprivate let userManageId:String = "sstooll"
fileprivate let kSweetTitle:String = "birthday"
...
class HaveViewController: LogThen {
    ...
    init(momentModel: LogHandyJSON, index: Int, type: GentleCandidatePhotoType) {
        ...
        if PrintingReactiveCompatible.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(noti_senseContent), y: 0)
                //: self.pageIndex = index
                self.pageIndex = index
                //: self.updataPageLabel()
                self.countoByObserve()
            }
        }
    }
    ...
    init(images: [UIImage], index: Int, type: GentleCandidatePhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiToolMessage.reversed(), encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        modleCrushClincher(modle: currenModel)
//        if myType == .normal {
//            setDownloadView()
//        }
    }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: noti_senseContent, height: appMineMessage)
        ...
        view.frame = CGRect(x: 0, y: 0, width: noti_senseContent, height: appMineMessage)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(noti_senseContent), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(noti_senseContent), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(noti_senseContent), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(ConnectVisitorViewDelegate.self, forCellWithReuseIdentifier: ConnectVisitorViewDelegate.className())
...
//    lazy var downloadBtn: WordOfAdviceThen = {
//        let btn = WordOfAdviceThen.init()
...
//        AsidePermissionTool.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: ExactlyHandyJSON = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func streamerSubmit(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.bringHomePublishGap(showMsg: (String(kPrivacyValue.suffix(6)) + "ucce" + userManageId.replacingOccurrences(of: "too", with: "fu") + kSweetTitle.replacingOccurrences(of: "birthday", with: "y")).localized)
                }
                //: } else {
            } else {
...
extension HaveViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func sameThe() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func belowView() {
        //: if  currenModel.sex==AdministratorThen.share.loginUserMode.sex {
        if currenModel.sex == AdministratorThen.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: ConnectVisitorViewDelegate = collectionView.dequeueReusableCell(withReuseIdentifier: ConnectVisitorViewDelegate.className(), for: indexPath) as! ConnectVisitorViewDelegate
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: ExactlyHandyJSON = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.nowTa(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.thinMonth(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.thinMonth(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: ConnectVisitorViewDelegate = cell as! ConnectVisitorViewDelegate
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / noti_senseContent)
        //: if myType == .normal {
        if myType == .normal {
