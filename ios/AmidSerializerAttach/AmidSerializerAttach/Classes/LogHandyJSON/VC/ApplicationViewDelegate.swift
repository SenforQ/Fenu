...
/*: "Cancel" :*/
fileprivate let k_premiumValue:String = "Cancelallow my soon addition"

/*: "Post" :*/
fileprivate let showColumnPatientId:[Character] = ["P","o","s","t"]

/*: "Upload failed" :*/
fileprivate let m_backKey:String = "series finish enhance thinUpload fa"
fileprivate let appQueryedData:String = "ilbirthd"

/*: "imgUrl" :*/
fileprivate let user_meUrl:[Character] = ["i","m","g","U","r","l"]

/*: "videoUrl" :*/
fileprivate let userLowName:[Character] = ["v","i","d","e","o","U","r","l"]

/*: "content" :*/
fileprivate let noti_armName:String = "connormalnt"

/*: "location" :*/
fileprivate let mainThanksKey:[Character] = ["l","o","c","a","t","i","o","n"]

/*: "Video review in progress" :*/
fileprivate let main_steerId:[Character] = ["V","i","d","e","o"," ","r","e","v","i","e","w"," ","i","n"," ","p","r","o","g"]
fileprivate let noti_dutyName:String = "rloops"

/*: "Shoot video at least 5 seconds" :*/
fileprivate let m_allName:String = "i a warm partner suiteShoo"
fileprivate let app_adjustKey:String = "deo at page mutual rapidly"
fileprivate let appPageRawUrl:[Character] = ["l","e","a","s","t"," ","5"," "]
fileprivate let mainSwitchdOthersValue:[Character] = ["s","e","c","o","n","d","s"]

/*: "Camera" :*/
fileprivate let mWriteChildId:[Character] = ["C","a","m","e","r","a"]

/*: "Photo" :*/
fileprivate let main_itemTitle:[Character] = ["P","h","o","t","o"]

/*: "Video" :*/
fileprivate let dreamYesPath:[Character] = ["V","i","d","e","o"]
...
protocol TowardMechanismControllerDelegate: NSObjectProtocol {
    // 发布成功
    //: func ShareSucceed()
    func shareRecognize()
}
...
class ApplicationViewDelegate: FlexibleViewController {
    //: weak var delegate: TalkingFreeViewControllerDelegate?
    weak var delegate: TowardMechanismControllerDelegate?
    ...
    private lazy var reqManager: LiteralReactiveCompatible = //: return LiteralReactiveCompatible()
        .init()
    //: }()
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(k_premiumValue.prefix(6))).localized, for: .normal)
    ...
    private lazy var postBtn: UIButton = {
        ...
        btn.setTitle((String(showColumnPatientId)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.tabOn(colors: UIColor.printColor(), size: CGSize(width: 70, height: 30)), for: .normal)
    ...
    private lazy var collectionView: GainFlowLayout = {
        //: let collectionView = FreeCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        let collectionView = GainFlowLayout(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        ...
        collectionView.deleteActionBlock = { [weak self] in
            ...
            if self.collectionView.imageArray.count == 0 {
                ...
                self.collectionView.contentType = PoorMirrorPath.nones
                //: self.videoPath = ""
                self.videoPath = ""
                //: self.coverPath = ""
                self.coverPath = ""
...
extension ApplicationViewDelegate {
    /// 发布动态
    ...
    func indexMoment() {
        ...
        reqManager.positiveFeedback(photos: self.collectionView.imageArray, content: content) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            ...
            NotificationCenter.default.post(name: show_styleIdent, object: self, userInfo: nil)
        }
    }
    ...
    func table() {
        ...
        ManagerRoundingPinpointTvReactiveCompatible.shared.doingExist(type: 1, coverPath: self.coverPath, videoPath: self.videoPath) { coverUrl, videoUrl, succeed in
            //: guard succeed else {
            guard succeed else {
                //: ProgressHUD.dismiss()
                VillageView.statusDismiss()
                //: self.func__showStatusBarErrorMsg(showMsg: "Upload failed".localized)
                self.iconThemeBringBackPlace(showMsg: (String(m_backKey.suffix(9)) + appQueryedData.replacingOccurrences(of: "birth", with: "e")).localized)
                //: return
                return
            }
            ...
            dict[(String(user_meUrl))] = coverUrl
            //: dict["videoUrl"] = videoUrl
            dict[(String(userLowName))] = videoUrl
            //: dict["content"] = content
            dict[(noti_armName.replacingOccurrences(of: "normal", with: "te"))] = content
            //: dict["location"] = ""
            dict[(String(mainThanksKey))] = ""

            //: TalkingMomentRequestTool.releaseVideoMoment(params: dict) {[weak self] succeed, result, errorModel in
            LiteralReactiveCompatible.coverCurrentTar(params: dict) { [weak self] succeed, _, _ in
                //: ProgressHUD.dismiss()
                VillageView.statusDismiss()
                //: guard let self = self else { return }
                guard let self = self else { return }
                ...
                self.bringHomePublishGap(showMsg: (String(main_steerId) + noti_dutyName.replacingOccurrences(of: "loop", with: "es")).localized)
                //: self.navigationController?.popViewController(animated: true)
                self.navigationController?.popViewController(animated: true)
                //: self.delegate?.ShareSucceed()
                self.delegate?.shareRecognize()
                //: NotificationCenter.default.post(name: POST_EXPLORE_SUCCESS_NOTIFICATION, object: self, userInfo: nil)
                NotificationCenter.default.post(name: show_styleIdent, object: self, userInfo: nil)
            }
        }
...
extension ApplicationViewDelegate {
    ...
    func lanceCamera() {
        ...
        AsidePermissionTool.goopBlock(true) { (isOpen: Bool) in
    ...
    func conceptAlbum() {
        ...
        AsidePermissionTool.replaceDown(true) { (isOpen: Bool) in
            ...
            let vc = GagePickTool.examine(maxCount: count, allowPhoto: true, allowVideo: false)
            ...
            vc.didFinishPickingVideoHandle = { [weak self] (_: UIImage?,
                    ...
                    GagePickTool.spreadInput(asset: assets) { filePath in
                        ...
                        if seconds >= 5 {
                            ...
                            self.cornerSegment(url: filePath!)
                            //: } else {
                        } else {
                            //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                                //: self.func__showStatusBarErrorMsg(showMsg: "Shoot video at least 5 seconds".localized)
                                self.iconThemeBringBackPlace(showMsg: (String(m_allName.suffix(4)) + "t vi" + String(app_adjustKey.prefix(7)) + String(appPageRawUrl) + String(mainSwitchdOthersValue)).localized)
                            }
                        }
                    }
            }
    ...
    func cornerSegment(url: URL) {
        //: let videoVC = TalkingVideoEditorController(maxDuration: 60.0, videoURL: url) { videoInfo in
        let videoVC = PerusalThen(maxDuration: 60.0, videoURL: url) { videoInfo in
...
extension ApplicationViewDelegate: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
        ...
        self.dismiss(animated: true) {
            ...
            if mediaType == kUTTypeMovie as String {
                ...
                if seconds >= 5 {
                    ...
                    self.cornerSegment(url: info[.mediaURL]! as! URL)
...
// MARK: - QuickViewDelegate

//: extension TalkingFreeViewController: FreeCollectionViewDelegate {
extension ApplicationViewDelegate: QuickViewDelegate {
    /// 选择图片
    ...
    func addBy() {
        ...
        if self.collectionView.contentType == .Photo {
            ...
            vc.aList(cellTitleList: [(String(mWriteChildId)).localized, (String(main_itemTitle)).localized])
            //: }else {
        } else {
            //: vc.initwithList(cellTitleList: ["Camera".localized, "Photo".localized, "Video".localized])
            vc.aList(cellTitleList: [(String(mWriteChildId)).localized, (String(main_itemTitle)).localized, (String(dreamYesPath)).localized])
        }
    ...
    func than(index: IndexPath) {
        ...
        let vc = HaveViewController(images: self.collectionView.imageArray, index: index.row, type: .publish)
        // 删除按钮block
    ...
    func dot() {
        ...
        let vc = InMigrationRecognizerDelegate(videoPath: self.videoPath)
        //: vc.modalPresentationStyle = .fullScreen
        vc.modalPresentationStyle = .fullScreen
        //: self.present(vc, animated: true)
        self.present(vc, animated: true)
...
extension ApplicationViewDelegate {
    ...
    func club() {
        ...
        collectionView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(appServerSensePath)
