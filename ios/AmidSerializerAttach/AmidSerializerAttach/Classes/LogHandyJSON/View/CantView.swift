...
fileprivate let notiExposureName:[UInt8] = [0x29,0x2e,0x29,0x34,0x68,0x23,0x2f,0x24,0x25,0x32,0x7a,0x69,0x60,0x28,0x21,0x33,0x60,0x2e,0x2f,0x34,0x60,0x22,0x25,0x25,0x2e,0x60,0x29,0x2d,0x30,0x2c,0x25,0x2d,0x25,0x2e,0x34,0x25,0x24]

private func fatalMissMethod(discount num: UInt8) -> UInt8 {
    return num ^ 64
}
...
class CantView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiExposureName.map{fatalMissMethod(discount: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: RaceProgressView = {
        ...
        let v = RaceProgressView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(unfinishedBusinessGesture(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(sum(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension CantView {
    ...
    @objc func unfinishedBusinessGesture(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func sum(gesture: UIPanGestureRecognizer) {
