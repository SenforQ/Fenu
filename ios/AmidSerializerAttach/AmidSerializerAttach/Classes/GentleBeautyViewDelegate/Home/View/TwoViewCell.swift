...
fileprivate let userBrandValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Welcome to my room" :*/
fileprivate let k_manTitle:[Character] = ["W","e","l","c","o","m","e"," ","t","o"," "]
fileprivate let showAideValue:String = "clear suspicion context ban rivermy room"

/*: "%@'s room" :*/
fileprivate let noti_attitudeValue:[Character] = ["%","@","\'","s"," ","r","o","o","m"]

/*: "CCCCCC" :*/
fileprivate let mainEnterSelectionValue:String = "cccccc"

/*: "#E1D0FC" :*/
fileprivate let mainPubliclyFormat:String = "pure range via#E1D0FC"

/*: "#FFE3E5" :*/
fileprivate let app_bronzeName:[Character] = ["#","F","F","E","3","E","5"]

/*: "get json error" :*/
fileprivate let noti_leafStr:[Character] = ["g","e","t"," ","j"]
fileprivate let userOpportunityMsg:String = "color bubble handson error"
...
class TwoViewCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userBrandValue.reversed(), encoding: .utf8)!)
    }
    ...
    var listModel: LaborPartyHandyJSON? {
        //: didSet {
        didSet {
            //: guard let listModel else { return }
            guard let listModel else { return }

            //: if String(listModel.uid) == AdministratorThen.share.loginUid {
            if String(listModel.uid) == AdministratorThen.share.loginUid {
                ...
                countryLab.text = (String(k_manTitle) + String(showAideValue.suffix(7))).localized
                //: countryLab.textColor = .appValueColor()
                countryLab.textColor = .portion()
                //: countryLab.font = UIFont.pingfangFont(type: .Medium, fontSize: 15)
                countryLab.font = UIFont.afterDisable(type: .Medium, fontSize: 15)
                ...
                countryLab.textColor = .fluent()
                //: countryLab.font = UIFont.pingfangFont(type: .Regular, fontSize: 15)
                countryLab.font = UIFont.afterDisable(type: .Regular, fontSize: 15)
                //: countryLab.snp.updateConstraints { make in
                countryLab.snp.updateConstraints { make in
            ...
            headPicImgV.overloadFinish(urlStr: listModel.pic)
            //: nameLab.text = "%@'s room".localizedArguments(listModel.name)
            nameLab.text = (String(noti_attitudeValue)).innerArguments(listModel.name)
            //: countryImgView.setUrlImage(urlStr: listModel.flag)
            countryImgView.overloadFinish(urlStr: listModel.flag)
            //: hotLab.text = "\(listModel.score)"
            hotLab.text = "\(listModel.score)"

            //: let hotWidth  = hotLab.sizeThatFits(CGSize(width: ScreenWidth, height: 20)).width
            let hotWidth = hotLab.sizeThatFits(CGSize(width: noti_senseContent, height: 20)).width
    ...
    private lazy var bgView: UIView = {
        ...
        v.layer.shadowColor = UIColor(hex: (mainEnterSelectionValue.uppercased()))?.withAlphaComponent(0.5).cgColor
    ...
    private lazy var bgvlayer: CAGradientLayer = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: noti_senseContent - 24, height: 76)
        //: layer.startPoint = CGPoint(x: 0, y: 0)
        layer.startPoint = CGPoint(x: 0, y: 0)
        //: layer.endPoint = CGPoint(x: 1, y: 0.5)
        layer.endPoint = CGPoint(x: 1, y: 0.5)
        //: layer.colors = [UIColor(hex: "#E1D0FC")?.cgColor, UIColor(hex: "#FFE3E5")?.cgColor]
        layer.colors = [UIColor(hex: (String(mainPubliclyFormat.suffix(7))))?.cgColor, UIColor(hex: (String(app_bronzeName)))?.cgColor]
    ...
    private lazy var liveSvga: SVGAPlayer = {
        ...
        do {
            ...
            let url = SampleEffectTool.default.joinWeight(type: .Party_List)
            ...
            printLog(message: (String(noti_leafStr) + String(userOpportunityMsg.suffix(9))))
        }
    ...
    private lazy var hotLab: UILabel = {
        ...
        lab.font = UIFont.afterDisable(type: .Medium, fontSize: 13)
