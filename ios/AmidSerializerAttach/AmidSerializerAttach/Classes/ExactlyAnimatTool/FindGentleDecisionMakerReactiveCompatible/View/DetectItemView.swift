...
/*: "#ffffff" :*/
fileprivate let k_budgetKey:String = "#ffffffcapture stick impression unite leaf"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let main_hungResentText:String = "/disfore check during"
fileprivate let notiDirtKey:[Character] = ["t","/","l","o","u","n","g","e","P","l","u","s","/","i","n","d","e","x",".","h","t","m","l"]

/*: "https://" :*/
fileprivate let mainGiftUrl:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://" :*/
fileprivate let user_jawMessage:String = "http://river mid"
...
class DetectItemView: UIView {
    ...
    let Broadcast_TextColor = (String(k_budgetKey.prefix(7)))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .imitationClotheStatement(fontSize: 12)
    ...
    convenience init(model: ExtentModelType?) {
        //: self.init()
        self.init()
    ...
    private func ignoreProcess() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            characterPremium(withUrl: danumuMsgModel.bgPic)
        }

        //: if danumuMsgModel.content.count > 0 {
        if danumuMsgModel.content.count > 0 {
            //: let parse = TUIChatParseTagHelper.init()
            let parse = BubbleTagHelper()
            ...
            var mutableAtt = parse.tarAnimal(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func characterPremium(withUrl url: String) {
        ...
        self.backgroundImageView.refreshHistory(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.soaring(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: ExtentModelType? {
        //: didSet {
        didSet {
            //: configModel()
            ignoreProcess()
    ...
    var totalTime: CGFloat {
        ...
        return (noti_senseContent + widthValue) * Broadcast_SpendTime / noti_senseContent
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / noti_senseContent
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / noti_senseContent
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(queryOnAction(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension DetectItemView {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func queryOnAction(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: DumpPushManager.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            DumpPushManager.share.belowStreet(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: DumpPushManager.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                DumpPushManager.share.animalToo(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(main_hungResentText.prefix(4)) + String(notiDirtKey))) {
                    //: DumpPushManager.share.func__pushToSubscribePageWebVC()
                    DumpPushManager.share.annualReward()
                    //: return
                    return
                }
                //: let httpsUrl = AdministratorThen.share.appConfigMode.urlH5Domain+url
                let httpsUrl = AdministratorThen.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(mainGiftUrl)), with: (String(user_jawMessage.prefix(7))))
                //: DumpPushManager.share.func__pushToWebVC(urlStr: httpUrl)
                DumpPushManager.share.noneControlInstall(urlStr: httpUrl)
            }
