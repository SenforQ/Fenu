...
fileprivate let dream_gageFairName:[UInt8] = [0x8a,0x8d,0x8a,0x97,0xcb,0x80,0x8c,0x87,0x86,0x91,0xd9,0xca,0xc3,0x8b,0x82,0x90,0xc3,0x8d,0x8c,0x97,0xc3,0x81,0x86,0x86,0x8d,0xc3,0x8a,0x8e,0x93,0x8f,0x86,0x8e,0x86,0x8d,0x97,0x86,0x87]

private func failurePrison(fare num: UInt8) -> UInt8 {
    return num ^ 227
}

/*: "#2DF2FF" :*/
fileprivate let user_subjectTitle:[Character] = ["#","2"]
fileprivate let userContrastValue:[Character] = ["D","F","2","F","F"]

/*: "Live" :*/
fileprivate let main_sunlightIdent:[Character] = ["L","i","v","e"]

/*: "img_home_shadow_icon" :*/
fileprivate let main_knowCollectionValue:String = "contain houseimg_h"
fileprivate let appAgainName:String = "sselected"
fileprivate let main_tingPath:String = "dow_iconpattern network media"

/*: "get json error" :*/
fileprivate let mComprehensiveText:String = "successful presentation copy blue insideget json "
fileprivate let userEndReplaceMessage:String = "ERROR"

/*: "#FF2348" :*/
fileprivate let showYouBecomeKey:[Character] = ["#","F","F","2","3","4"]
fileprivate let showMidnightStr:String = "body"

/*: "#35C759" :*/
fileprivate let mainDealUrl:String = "#35C759give only"

/*: "mfBean" :*/
fileprivate let kControlCookingPath:String = "option pin simply memorymfBean"

/*: "points\n≈ $" :*/
fileprivate let mainMarginMsg:[Character] = ["p","o","i","n","t"]
fileprivate let appReplacePepStr:String = "s\n≈ $"
...
public var dream_withoutValue: Int {
    //: return AdministratorThen.share.appConfigMode.countdown1 + AdministratorThen.share.appConfigMode.countdown2
    return AdministratorThen.share.appConfigMode.countdown1 + AdministratorThen.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class ExactlyThen: AccountReactiveCompatible {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= AdministratorThen.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(dream_withoutValue - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                regenerate()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(AdministratorThen.share.loginUserMode.mf_bean) > 0 {
        if Double(AdministratorThen.share.loginUserMode.mf_bean) > 0 {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dream_gageFairName.map{failurePrison(fare: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: WordOfAdviceThen = {
        ...
        let image = UIImage.itsyBitsy(color: UIColor(hex: (String(user_subjectTitle) + String(userContrastValue)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.enter(), for: .normal)
        img.setImage(image.enter(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(main_sunlightIdent)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.scribeNumber(name: (String(main_knowCollectionValue.suffix(5)) + "ome_" + appAgainName.replacingOccurrences(of: "selected", with: "ha") + String(main_tingPath.prefix(8))))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(mComprehensiveText.suffix(9)) + userEndReplaceMessage.lowercased()))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(showYouBecomeKey) + showMidnightStr.replacingOccurrences(of: "body", with: "8")))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(mainDealUrl.prefix(7))))
...
extension ExactlyThen {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func perspective() -> ExactlyThen {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = ExactlyThen(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = AdviceLandscapeReactiveCompatible.shared.liveMiniPosition
    ...
    func regenerate() {
        ...
        liveStatusImgV.isHidden = !TapBecauseLiveManager.resolutionShared().isLive
...
extension ExactlyThen {
    ...
    private func topOn() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(removeAfterSearch(notification:)),
    ...
    @objc private func removeAfterSearch(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(kControlCookingPath.suffix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        existent(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func existent(mfBean: Double) {
        //: let nowBean = AdministratorThen.share.loginUserMode.mf_bean + mfBean
        let nowBean = AdministratorThen.share.loginUserMode.mf_bean + mfBean
        //: AdministratorThen.share.loginUserMode.mf_bean = nowBean
        AdministratorThen.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((String(mainMarginMsg) + appReplacePepStr).localized) \(divisor)"
