...
fileprivate let kEgoMsg:[UInt8] = [0x3a,0x3f,0x3a,0x45,0xf9,0x34,0x40,0x35,0x36,0x43,0xb,0xfa,0xf1,0x39,0x32,0x44,0xf1,0x3f,0x40,0x45,0xf1,0x33,0x36,0x36,0x3f,0xf1,0x3a,0x3e,0x41,0x3d,0x36,0x3e,0x36,0x3f,0x45,0x36,0x35]

fileprivate func localOffly(animal num: UInt8) -> UInt8 {
...
@objc protocol VanguardObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func searchion()

    //: @objc optional func endScroll()
    @objc optional func streamInner()
}

//: class TalkingDanmuMsgListTableView: UIView {
class AdaptListTableView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kEgoMsg.map{localOffly(animal: $0)}, encoding: .utf8)!)
    }
...
extension AdaptListTableView {
    ...
    func dropHair() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(relapse), userInfo: nil, repeats: true)
    }
...
extension AdaptListTableView {
    ...
    func untested(msgModel: AccountMeasurable?) {
    ...
    func setOff() {
        ...
        if AllHeight > user_intervalMsg {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < user_intervalMsg {
                ...
                self.msgTableView.height = user_intervalMsg
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = user_intervalMsg - AllHeight
    ...
    func jawPending(inPending: Bool) {
...
extension AdaptListTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func imminent(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.streamInner?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(userAskMessage)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = CantBassCell.outsideLimited(tableView: tableView, msg: msgModel, indexPath: indexPath)
