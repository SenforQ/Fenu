...
/*: "#BAFBFF" :*/
fileprivate let userSpanFormat:String = "#BAFBFFflow challenge"
...
class AdaptTaThen: NSObject {
    ...
    var model = DetectFairlyModelType()

    //: func caculateMsgHeight(model: TalkingVideoCallDanmuModel) -> TalkingVideoCallDanmuModel {
    func versionMake(model: DetectFairlyModelType) -> DetectFairlyModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func counterpolate(model: DetectFairlyModelType) -> DetectFairlyModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func characteriseComment() {
        //: let paraStyle = paragraphStyle()
        let paraStyle = fillIn()
        ...
        let name = bagIn(text: firstStr, font: UIFont.imitationClotheStatement(fontSize: 14), color: UIColor(hex: (String(userSpanFormat.prefix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = bagIn(text: self.model.content, font: UIFont.imitationClotheStatement(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        writtenMatterWithoutName(attribText: self.model.msgAttribText)
    }
    ...
    func radioBeacon() {
        ...
        let content = bagIn(text: self.model.transContent, font: UIFont.imitationClotheStatement(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func fillIn() -> NSMutableParagraphStyle {
    ...
    func bagIn(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = fillIn()
        //: return attribute
        return attribute
    }
    ...
    func writtenMatterWithoutName(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = mBarStr - 36
