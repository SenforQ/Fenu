...
fileprivate let mainEffectStr:[UInt8] = [0x61,0x66,0x61,0x6c,0x20,0x5b,0x67,0x5c,0x5d,0x6a,0x32,0x21,0x18,0x60,0x59,0x6b,0x18,0x66,0x67,0x6c,0x18,0x5a,0x5d,0x5d,0x66,0x18,0x61,0x65,0x68,0x64,0x5d,0x65,0x5d,0x66,0x6c,0x5d,0x5c]

fileprivate func femaleRelation(sub num: UInt8) -> UInt8 {
    let value = Int(num) + 8
    if value > 255 {
...
fileprivate let appImportantIdent:[Character] = ["T","R","T","C"," ","视","频","\u{901a}","话","向"]
fileprivate let k_overStr:String = "SDK发\u{9001}采"
fileprivate let showRetKey:[Character] = ["集","的","视","频","\u{5e27}","图","片",":"," "]

/*: "LIVE 直播向SDK发送采集的视频帧图片:  :*/
fileprivate let main_turnAgreePath:[Character] = ["L","I","V","E"," ","直","播","向","S","D","K","发","送","采","集","的","视","频","帧","\u{56fe}","片",":"]
fileprivate let notiVillageId:String = " "
...
class TowardViewDelegete: UIView {
    // 直播推流类
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainEffectStr.map{femaleRelation(sub: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var captureView: AidView = {
...
extension TowardViewDelegete {
    ...
    func unit(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
        ...
        if self.captureView.superview == nil {
            ...
            self.captureView.stCamera.frontPush()
        }
        //: self.captureView.setupVideoConversationPreviewSize(isSmallWindow)
        self.captureView.view(isSmallWindow)
    }
    ...
    func rootOrigin() {
        ...
        self.captureView.stCamera.position()
    ...
    func smartPutBroadcast(isFront _: Bool) {
        //: guard SenseTime_Use == true else { return }
        guard user_progressId == true else { return }
        //: guard self.captureView.stCamera != nil else {
        guard self.captureView.stCamera != nil else {
...
extension TowardViewDelegete: QuantityerestedDelegete {
    // TRTC推流
    //: func stVideoBeautyView(_ beautyView: STFilterView!, didProcessVideoSampleBuffer sampleBuffer: CVPixelBuffer!) {
    func hemAndHawBuffer(_: AidView!, directContrast sampleBuffer: CVPixelBuffer!) {
        ...
        if trtcCallCount >= logReportInterval {
            ...
            if let image = tickRead(sampleBuffer) {
                //: UploadLogTool.writeLog(msg: "TRTC 视频通话向SDK发送采集的视频帧图片: \(image.size)")
                ProblemBenchPoundThen.panelDetect(msg: (String(appImportantIdent) + k_overStr + String(showRetKey)) + "\(image.size)")
            }
            //: trtcCallCount = 0
            trtcCallCount = 0
        }
    ...
    func past(_: AidView!, surcharge sampleBuffer: CMSampleBuffer!) {
        //: if livePusher != nil {
        if livePusher != nil {
            ...
            if liveCallCount >= logReportInterval {
                ...
                if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
                    ...
                    if let image = tickRead(imageBuffer) {
                        //: UploadLogTool.writeLog(msg: "LIVE 直播向SDK发送采集的视频帧图片: \(image.size)")
                        ProblemBenchPoundThen.panelDetect(msg: (String(main_turnAgreePath) + notiVillageId.capitalized) + "\(image.size)")
                    }
                }
    ...
    func tickRead(_ imageBuffer: CVImageBuffer) -> UIImage? {
