...
fileprivate let appFirmRandomStr:[UInt8] = [0x4d,0x4a,0x4d,0x50,0xc,0x47,0x4b,0x40,0x41,0x56,0x1e,0xd,0x4,0x4c,0x45,0x57,0x4,0x4a,0x4b,0x50,0x4,0x46,0x41,0x41,0x4a,0x4,0x4d,0x49,0x54,0x48,0x41,0x49,0x41,0x4a,0x50,0x41,0x40]
...
enum CantColumnConvertible: Int {
    //: case TimeGap = 0
...
@objc protocol SampleViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func startOut()

    //: @objc optional func endScroll()
    @objc optional func broadInner()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class EscapeViewDelegate: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(since))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appFirmRandomStr.map{$0^36}, encoding: .utf8)!)
    }
...
extension EscapeViewDelegate {
    ...
    func exposureTimer() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(timerListen), userInfo: nil, repeats: true)
    }
...
extension EscapeViewDelegate {
    ...
    func freshRead(msgModel: DetectFairlyModelType?) {
    ...
    func number() {
        ...
        if AllHeight > k_sendIdent {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < k_sendIdent {
                ...
                self.msgTableView.height = k_sendIdent
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = k_sendIdent - AllHeight
        ...
        policy(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func policy(animated: Bool) {
...
extension EscapeViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func reduceSmart(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.broadInner?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(user_sessionMessage)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = SelfLoadingThen.titles(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension EscapeViewDelegate {
    ...
    func lockMove() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: mBarStr, height: k_sendIdent)
    }
}
