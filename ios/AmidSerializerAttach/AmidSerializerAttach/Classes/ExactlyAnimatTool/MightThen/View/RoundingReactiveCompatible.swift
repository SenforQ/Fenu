...
fileprivate let dream_phaseReachKey:[UInt8] = [0x18,0x1f,0x18,0x5,0x59,0x12,0x1e,0x15,0x14,0x3,0x4b,0x58,0x51,0x19,0x10,0x2,0x51,0x1f,0x1e,0x5,0x51,0x13,0x14,0x14,0x1f,0x51,0x18,0x1c,0x1,0x1d,0x14,0x1c,0x14,0x1f,0x5,0x14,0x15]

private func vidTone(exception num: UInt8) -> UInt8 {
    return num ^ 113
}

/*: "#FFD13A" :*/
fileprivate let dream_makeupOperateParaId:String = "computer tap for#FFD13A"

/*: "gift_gb_umnad_1" :*/
fileprivate let m_versionIdent:String = "private pretty player king arrivegift_g"
fileprivate let showGoldenStr:String = "portrait openly example row bubblead_1"

/*: "icon_circle_pic_default" :*/
fileprivate let show_developShPath:String = "icon_include para coordinator don landmark"
fileprivate let appPlayRequireName:String = "_pic_dalongside determine yield pass wife"
fileprivate let dream_involveSlideKey:[Character] = ["e","f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let kReceiveName:[Character] = ["O","p","e","n"," ","l","u"]
fileprivate let k_activeKey:String = "cky blthis local sweet"

/*: "Sent " :*/
fileprivate let mainErrorIdent:[Character] = ["S","e","n","t"," "]

/*: "PingFangSC-Semibold" :*/
fileprivate let user_minimumName:String = "boy op admin dismiss stripPingF"
fileprivate let userSuiteStr:[Character] = ["m","i","b","o","l","d"]

/*: "Bombard" :*/
fileprivate let user_visitorFormat:[Character] = ["B","o","m","b","a","r","d"]
...
// __CLOSE_PRINT__
//
//  RoundingReactiveCompatible.swift
//  AbroadTalking
//
...
class RoundingReactiveCompatible: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = RaceMeasurable()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [SprinklerSystemTransformable()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dream_phaseReachKey.map{vidTone(exception: $0)}, encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(dream_makeupOperateParaId.suffix(7))))
    ...
    private lazy var pluralTip: BequestThen = {
    ...
    private lazy var countLb: BequestThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = BequestThen()
...
extension RoundingReactiveCompatible {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func hypothesisModel(model: RaceMeasurable) {
        ...
        self.bgImage.image = UIImage.scribeNumber(name: (String(m_versionIdent.suffix(6)) + "b_umn" + String(showGoldenStr.suffix(4))))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.scribeNumber(name: (String(show_developShPath.prefix(5)) + "circle" + String(appPlayRequireName.prefix(6)) + String(dream_involveSlideKey)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == DegreeTransformable.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(kReceiveName) + String(k_activeKey.prefix(6)) + "ind-box").localized
            //: }else {
        } else {
        ...
        self.avatarImage.refreshHistory(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.refreshHistory(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(mainErrorIdent)).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(user_minimumName.suffix(5)) + "angSC-Se" + String(userSuiteStr)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = listSkip(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(user_minimumName.suffix(5)) + "angSC-Se" + String(userSuiteStr)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = listSkip(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.attributed(texts: strs, colors: colors, fonts: fonts)
        ...
        minute(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func minute(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(user_minimumName.suffix(5)) + "angSC-Se" + String(userSuiteStr)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = listSkip(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(user_minimumName.suffix(5)) + "angSC-Se" + String(userSuiteStr)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = listSkip(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.fruitThumb(150, 77, 20, 1), UIColor.white, UIColor.fruitThumb(150, 77, 20, 1)]
        ...
        attText = attText.attributed(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.draftTitle(attributedText: attText)
        ...
        countLb.exposureFrame(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func oppositeBold(giftMsgModel: RaceMeasurable) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(chore), object: nil)
        ...
        self.perform(#selector(chore), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func fiscalHasten(model _: RaceMeasurable, animate _: Bool) {
...
extension RoundingReactiveCompatible {
    ...
    func listSkip(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension RoundingReactiveCompatible {
    ...
    private func quantityoSubviews() {
        //: let height = 46.0
        let height = 46.0
        ...
        font = listSkip(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.attributed(texts: [(String(user_visitorFormat)).localized], colors: [UIColor.fruitThumb(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.exposureFrame(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.draftTitle(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.exposureFrame(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
