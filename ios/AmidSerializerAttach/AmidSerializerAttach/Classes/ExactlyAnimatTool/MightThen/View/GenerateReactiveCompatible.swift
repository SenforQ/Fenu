...
fileprivate let notiAideUrl:[UInt8] = [0x30,0x35,0x30,0x3b,0xef,0x2a,0x36,0x2b,0x2c,0x39,0x1,0xf0,0xe7,0x2f,0x28,0x3a,0xe7,0x35,0x36,0x3b,0xe7,0x29,0x2c,0x2c,0x35,0xe7,0x30,0x34,0x37,0x33,0x2c,0x34,0x2c,0x35,0x3b,0x2c,0x2b]

fileprivate func giveContextWipe(calendar num: UInt8) -> UInt8 {
...
/*: "#171717" :*/
fileprivate let showAideInviteUrl:String = "#lovelylovelylovely"

/*: "icon_cameraoff" :*/
fileprivate let app_merelyText:String = "allowon"
fileprivate let dream_meetingUrl:String = "eraodetaileddetailed"
...
class GenerateReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.fruitThumb(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiAideUrl.map{giveContextWipe(calendar: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (showAideInviteUrl.replacingOccurrences(of: "lovely", with: "17")))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.scribeNumber(name: (app_merelyText.replacingOccurrences(of: "allow", with: "ic") + "_cam" + dream_meetingUrl.replacingOccurrences(of: "detailed", with: "f")))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension GenerateReactiveCompatible {
    ...
    func countoCamera(urlStr: String?, isOpenCamera: StopCameraStatus) {
...
extension GenerateReactiveCompatible: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func like(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, noti_senseContent - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, appMineMessage - self.frame.size.height * 0.5)
    ...
    @objc private func contextSource() {
        //: if self.block != nil {
        if self.block != nil {
            //: self.block!()
            self.block!()
...
extension GenerateReactiveCompatible {
    ...
    private func strokeCounteraction() {
        // 切换视角手势
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(exchangeRenderContainer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(contextSource))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(like(pan:)))
        //: pan.delegate = self
        pan.delegate = self
