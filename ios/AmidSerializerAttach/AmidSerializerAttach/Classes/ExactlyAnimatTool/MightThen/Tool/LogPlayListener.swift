...
/*: "call_bgm" :*/
fileprivate let main_gainIdent:String = "call_bgmcrush certain"

/*: "call_response_bgm" :*/
fileprivate let dream_supplyHandName:[Character] = ["c","a","l","l","_","r","e","s","p","o","n","s","e","_","b","g","m"]
...
class LogPlayListener: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = WarningSignedNumeric.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func sink(type: WarningSignedNumeric) -> LogPlayListener {
        //: let player = TalkingCallBGMPlayer()
        let player = LogPlayListener()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.homePath()
...
extension LogPlayListener {
    ...
    private func vesselWithoutNotify(notify: Notification) {
    ...
    private func ratingPosition(notify: Notification) {
    ...
    private func homePath() {
        //: var fileName = "call_bgm"
        var fileName = (String(main_gainIdent.prefix(8)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(dream_supplyHandName))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = SampleEffectTool.default.atm(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension LogPlayListener: TXVodPlayListener {
