...
fileprivate let notiGatherKey:String = "Documreason guide"
fileprivate let appStickPath:[Character] = ["/","U","s","e","r","/","v"]
fileprivate let dream_representativeEntryKey:String = "process fade also bucketoice/"

/*: "Documents/User/record/" :*/
fileprivate let userBodyContent:[Character] = ["D","o","c","u","m","e","n","t","s","/","U","s","e","r","/","r","e","c","o","r","d"]
fileprivate let dream_magnitudeMessage:String = "fruit"

/*: "cancelled" :*/
fileprivate let userConversationIdent:String = "calinke"
fileprivate let mainLevelMessage:String = "proposaled"

/*: .wav" :*/
fileprivate let appSupplyWindData:[Character] = [".","w","a","v"]
...
// __CLOSE_PRINT__
//
//  SprechgesangObjectProtocol.swift
...
typealias OneKeyGreetDownloadBlock = (_ model: WaitTaskModel) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol SprechstimmeObjectProtocol: NSObjectProtocol {
    /// 下载完成
    ...
    func reportTo(model: WaitTaskModel)
    /// 过期语音消息，服务器缓存时间为7天
...
extension SprechgesangObjectProtocol {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func anTop(_ tasksArray: [WaitTaskModel]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.contactFun()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(notiGatherKey.prefix(5)) + "ents" + String(appStickPath) + String(dream_representativeEntryKey.suffix(5)))) {
                //: model.zipFilePath = "\(GatherCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(GatherCacheDefine.doFlame())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(userBodyContent) + dream_magnitudeMessage.replacingOccurrences(of: "fruit", with: "/"))) {
                //: model.zipFilePath = "\(GatherCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(GatherCacheDefine.contract())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            FireMsgTable.scheduleVoice(msgId: model.taskId, status: "\(TaskScalar.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func publishHandle(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = WaitTaskModel.aneModel(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? WaitTaskModel()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(notiGatherKey.prefix(5)) + "ents" + String(appStickPath) + String(dream_representativeEntryKey.suffix(5)))) {
            //: taskModel.zipFilePath = "\(GatherCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(GatherCacheDefine.doFlame())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(userBodyContent) + dream_magnitudeMessage.replacingOccurrences(of: "fruit", with: "/"))) {
            //: taskModel.zipFilePath = "\(GatherCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(GatherCacheDefine.contract())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension SprechgesangObjectProtocol: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive _: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = WaitTaskModel.popularPath(path: taskModel.zipFilePath)
    ...
    func urlSession(_: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.reportTo(model: taskModel)
    }
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (userConversationIdent.replacingOccurrences(of: "link", with: "nc") + mainLevelMessage.replacingOccurrences(of: "proposal", with: "ll")) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    para(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.via(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                para(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.directionComplete(url: task.taskDescription ?? "")
        }
...
extension SprechgesangObjectProtocol {
    ...
    func para(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(TaskScalar.pause.rawValue)" : "\(TaskScalar.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            FireMsgTable.scheduleVoice(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.downloadModel(model: taskModel)
            }
    ...
    func via(url: String) {
        ...
        case .chatRecord:
            ...
            FireMsgTable.advantageId(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.copernicanSystem(model: taskModel)
    ...
    func directionComplete(url: String) {
        //: guard let taskModel = self.downloadingModelDic[url] else { return }
        guard let taskModel = self.downloadingModelDic[url] else { return }
        //: self.downloadingModelDic.removeValue(forKey: url)
        self.downloadingModelDic.removeValue(forKey: url)
        ...
        let size = WaitTaskModel.popularPath(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                FireMsgTable.scheduleVoice(msgId: taskModel.taskId, status: "\(TaskScalar.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(appSupplyWindData))
            ...
            self.lessComplete(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.sufficientCommand(model: taskModel)
            ...
            case .chatRecord:
                ...
                FireMsgTable.scheduleVoice(msgId: taskModel.taskId, status: "\(TaskScalar.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.itContrastModel(model: taskModel)
    ...
    func lessComplete(at path: String, withKeyId keyId: String, withTypeId typeId: CandidateInsetTarget, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(appSupplyWindData))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.birthOff()
                    ...
                    if typeId == .chatRecord {
                        ...
                        FireMsgTable.scheduleVoice(msgId: keyId, filePath: newPath)
                    }
