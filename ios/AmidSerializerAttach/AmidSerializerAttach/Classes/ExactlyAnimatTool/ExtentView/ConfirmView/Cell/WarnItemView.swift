...
/*: "#ffffff" :*/
fileprivate let mBalanceId:[Character] = ["#","f","f","f","f","f","f"]

/*: "earned" :*/
fileprivate let appMeetContainerUrl:[Character] = ["e","a","r","n","e","d"]

/*: "Coins in" :*/
fileprivate let mainSpecialName:[Character] = ["C","o","i","n","s"," ","i","n"]

/*: "Error:  :*/
fileprivate let m_breadMessage:String = "native exit quoteError:"
fileprivate let kHesitateName:String = " "

/*: "#C946D2" :*/
fileprivate let showClearlyTreatIdent:[Character] = ["#","C","9","4","6","D","2"]

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let dream_supplyFormat:String = "broad liebtn_"
fileprivate let k_becomeCommitPath:String = "_floatreplacement accept new dark"
fileprivate let mAlreadyNumberUrl:String = "word provoke pop_nor"
...
class WarnItemView: UIView {
    ...
    private let TextColor = (String(mBalanceId))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: AdviceFloatingScreenModel?) {
        //: self.init()
        self.init()
    ...
    private func actionPositive() {
        ...
        if danumuMsgModel.headPic.count > 0 {
            ...
            headAnimatedImageView.overloadFinish(urlStr: danumuMsgModel.headPic)
        }
        //: if danumuMsgModel.bgPic.count > 0 {
        if danumuMsgModel.bgPic.count > 0 {
            //: updateBackground(withUrl: danumuMsgModel.bgPic)
            know(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (String(appMeetContainerUrl))) {
                ...
                if let range = attrStr.string.range(of: (String(mainSpecialName))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.afterDisable(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(m_breadMessage.suffix(6)) + kHesitateName.capitalized) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func know(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.refreshHistory(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.soaring(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: AdviceFloatingScreenModel? {
        //: didSet {
        didSet {
            //: configModel()
            actionPositive()
    ...
    var totalTime: CGFloat {
        ...
        return (noti_senseContent + widthValue) * SpendTime / noti_senseContent
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / noti_senseContent
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / noti_senseContent
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(showClearlyTreatIdent)))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.scribeNumber(name: (String(dream_supplyFormat.suffix(4)) + "chat" + String(k_becomeCommitPath.prefix(6)) + "S_userBK" + String(mAlreadyNumberUrl.suffix(4))))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(premium(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.imitationClotheStatement(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension WarnItemView {
    /// 点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func premium(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = AdministratorThen.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = AdministratorThen.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: DumpPushManager.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        DumpPushManager.share.noneControlInstall(urlStr: httpsUrl, webConfig: config)
    }
}
