...
fileprivate let mainCommitName:[UInt8] = [0x96,0x91,0x96,0x8b,0xd7,0x9c,0x90,0x9b,0x9a,0x8d,0xc5,0xd6,0xdf,0x97,0x9e,0x8c,0xdf,0x91,0x90,0x8b,0xdf,0x9d,0x9a,0x9a,0x91,0xdf,0x96,0x92,0x8f,0x93,0x9a,0x92,0x9a,0x91,0x8b,0x9a,0x9b]

private func gradeHeartKeep(close num: UInt8) -> UInt8 {
    return num ^ 255
}

/*: "all" :*/
fileprivate let app_mentionId:String = "awitch"
...
class PositionThen: PoorMsgCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainCommitName.map{gradeHeartKeep(close: $0)}, encoding: .utf8)!)
    }
...
extension PositionThen {
    ...
    override func fieldWith(with data: PoorThen) {
        ...
        if giftData.msgModel.toUser.uid == (app_mentionId.replacingOccurrences(of: "witch", with: "ll")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.scribeNumber(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.overloadFinish(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
