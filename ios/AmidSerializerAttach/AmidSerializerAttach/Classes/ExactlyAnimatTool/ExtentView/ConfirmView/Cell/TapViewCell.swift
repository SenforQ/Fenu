...
fileprivate let k_nurseKey:[UInt8] = [0xb,0xc,0xb,0x16,0x4a,0x1,0xd,0x6,0x7,0x10,0x58,0x4b,0x42,0xa,0x3,0x11,0x42,0xc,0xd,0x16,0x42,0x0,0x7,0x7,0xc,0x42,0xb,0xf,0x12,0xe,0x7,0xf,0x7,0xc,0x16,0x7,0x6]

private func roadSafe(split num: UInt8) -> UInt8 {
    return num ^ 98
}

/*: "icon_chat_lock" :*/
fileprivate let user_feedbackSpecialText:String = "itrimn"
fileprivate let show_linkFormat:String = "loautomatic"

/*: "icon_chat_burned" :*/
fileprivate let kVesselValue:String = "ichugen"
fileprivate let main_brightIdent:String = "urnlive"

/*: "icon_chat_burned_shan" :*/
fileprivate let kAgainstSearchLovelyName:[Character] = ["i","c","o","n","_"]
fileprivate let mRoadPath:String = "byhat"
fileprivate let noti_proposalFormat:String = "contact into condition fastned_"

/*: "Already burned" :*/
fileprivate let m_headIdent:String = "Already sometime express"
fileprivate let dream_refTitle:String = "burcarrierd"
...
class TapViewCell: PoorMsgCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_nurseKey.map{roadSafe(split: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.scribeNumber(name: (user_feedbackSpecialText.replacingOccurrences(of: "trim", with: "co") + "_chat_" + show_linkFormat.replacingOccurrences(of: "automatic", with: "ck")))
    ...
    lazy var burnedImagV: WordOfAdviceThen = {
        ...
        imgV.setBackgroundImage(UIImage.scribeNumber(name: (kVesselValue.replacingOccurrences(of: "huge", with: "o") + "_chat_b" + main_brightIdent.replacingOccurrences(of: "live", with: "ed"))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.scribeNumber(name: (String(kAgainstSearchLovelyName) + mRoadPath.replacingOccurrences(of: "by", with: "c") + "_bur" + String(noti_proposalFormat.suffix(4)) + "shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(m_headIdent.prefix(8)) + dream_refTitle.replacingOccurrences(of: "carrier", with: "ne")).localized, for: .normal)
...
extension TapViewCell {
    //: override func fill(with data: TCommonCellData) {
    override func fieldWith(with data: PoorThen) {
        //: super.fill(with: data)
        super.fieldWith(with: data)
        //: guard let imageCellData = data as? GageBubbleCellData else { return }
        guard let imageCellData = data as? GageBubbleCellData else { return }
        ...
        let intimatePhoto: EffectModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            last(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            last(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: GageBubbleCellData, urlStr: String?, isMosaic: Bool) {
    func last(data: GageBubbleCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.icon(image, sumerest: 90)
                        self.photoView.image = UIImage.icon(image, sumerest: 90)
                        //: } else {
                    } else {
            ...
            demonstratePure(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.overloadFinish(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func demonstratePure(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.executeDown(urlStr: urlStr) { [weak self] image in
