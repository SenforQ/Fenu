...
fileprivate let show_problemUrl:[UInt8] = [0xea,0xed,0xea,0xf7,0xab,0xe0,0xec,0xe7,0xe6,0xf1,0xb9,0xaa,0xa3,0xeb,0xe2,0xf0,0xa3,0xed,0xec,0xf7,0xa3,0xe1,0xe6,0xe6,0xed,0xa3,0xea,0xee,0xf3,0xef,0xe6,0xee,0xe6,0xed,0xf7,0xe6,0xe7]

private func enjoySubmit(conduct num: UInt8) -> UInt8 {
    return num ^ 131
}

/*: "[未知消息]" :*/
fileprivate let m_darkUrl:String = "[未知消\u{606f}"
fileprivate let userAbsolutePath:String = "]"

/*: "extra" :*/
fileprivate let appSquareMsg:String = "extrop"

/*: "#0084FF" :*/
fileprivate let m_becomeStr:String = "#multiple"
fileprivate let app_performMessage:String = "084FFprocessor new"

/*: "#FFEE4B" :*/
fileprivate let show_positiveId:[Character] = ["#","F","F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let noti_betweenText:String = "<at>@[hair send bot side tone"
fileprivate let appDitDocName:String = "?</atsurvival foster forward ego"
fileprivate let user_publiclyMessage:[Character] = [">"]

/*: "jumpKey" :*/
fileprivate let show_mediaData:String = "jumpKeyscore imitation direct don"

/*: "user" :*/
fileprivate let dreamBassData:String = "udateer"

/*: "uid" :*/
fileprivate let appMemoryBreadData:[UInt8] = [0x6c,0x60,0x5b]

fileprivate func onPose(resource num: UInt8) -> UInt8 {
...
/*: "#16D073" :*/
fileprivate let notiPhoneNationalPath:[Character] = ["#","1","6","D","0"]
fileprivate let app_premiumAboveName:String = "73"

/*: "#F95644" :*/
fileprivate let noti_helpTitle:String = "invite artifact lay price#F95644"
...
//: @objcMembers public class ConnectThen: TalkingChatMsgBaseCellData {
@objcMembers public class ConnectThen: MarkCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_problemUrl.map{enjoySubmit(conduct: $0)}, encoding: .utf8)!)
    }
    ...
    override public var msgModel: InformedConsentModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (m_darkUrl + userAbsolutePath.capitalized)
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = accountNow(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func accountNow(msgInfoModel: TingModel) -> NSMutableAttributedString {
        ...
        guard dic[(appSquareMsg.replacingOccurrences(of: "op", with: "a"))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.manualColor() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (m_becomeStr.replacingOccurrences(of: "multiple", with: "0") + String(app_performMessage.prefix(5))))! : UIColor(hex: (String(show_positiveId)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(noti_betweenText.prefix(6)) + "\\S\\s]+" + String(appDitDocName.prefix(5)) + String(user_publiclyMessage)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(show_mediaData.prefix(7)))].stringValue == (dreamBassData.replacingOccurrences(of: "date", with: "s")) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: appMemoryBreadData.map{onPose(resource: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: appMemoryBreadData.map{onPose(resource: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func pageName() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(notiPhoneNationalPath) + app_premiumAboveName.capitalized))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(noti_helpTitle.suffix(7))))!
        }
