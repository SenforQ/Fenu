...
fileprivate let appSpendTopicData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#AAAAAA" :*/
fileprivate let main_completeUrl:String = "#AAAAAdistance bring success apartment her"
fileprivate let appSubjectValue:[Character] = ["A"]

/*: "btn_chatsettings_unchoice" :*/
fileprivate let appEnhanceBanName:String = "btn_cbutton limit line miracle favorite"
fileprivate let k_platMessage:String = "tinamid"
fileprivate let m_healthyData:[Character] = ["s","_","u","n","c","h","o","i","c","e"]

/*: "btn_chatsettings_choiced" :*/
fileprivate let app_effectivelyPowText:[Character] = ["b","t","n","_","c","h","a","t","s","e","t","t","i","n","g","s","_","c","h","o","i"]
fileprivate let mainSlowImageKey:[Character] = ["c","e","d"]

/*: "Top" :*/
fileprivate let mainSocialName:[Character] = ["T","o","p"]

/*: "Save" :*/
fileprivate let app_countText:String = "Savepastor civil ship terms"

/*: "Cancel" :*/
fileprivate let userHalfName:[Character] = ["C","a","n","c","e","l"]
...
class SampleThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appSpendTopicData.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + main_showUrl
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: appMineMessage - height, width: noti_senseContent, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.dimensionSixth(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.landmarkBalance()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(main_completeUrl.prefix(6)) + String(appSubjectValue)))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: WordOfAdviceThen = {
        ...
        btn.setImage(UIImage.scribeNumber(name: (String(appEnhanceBanName.prefix(5)) + "hatset" + k_platMessage.replacingOccurrences(of: "amid", with: "g") + String(m_healthyData))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.scribeNumber(name: (String(app_effectivelyPowText) + String(mainSlowImageKey))), for: .selected)
        ...
        btn.setTitle((String(mainSocialName)).localized, for: .normal)
        //: btn.spaceBetweenTitleAndImage = 6
        btn.spaceBetweenTitleAndImage = 6
        //: btn.addTarget(self, action: #selector(topButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(positiveAfter), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(app_countText.prefix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(snapMatch), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(userHalfName)).localized, for: .normal)
...
extension SampleThen: YYTextViewDelegate {
...
extension SampleThen {
    /// 保存按钮点击事件
    //: @objc private func saveButtonClick() {
    @objc private func snapMatch() {
        //: saveBtn.isEnabled = false
        saveBtn.isEnabled = false
        //: let isTop = topBtn.isSelected ? 1 : 2
        let isTop = topBtn.isSelected ? 1 : 2
        //: TalkingChatRequestTool.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        ShrinkReactiveCompatible.theMinCompletion(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
            ...
            self.dismissBlock?(self.textView.text, self.topBtn.isSelected ? 1 : 2)
            //: self.dismiss()
            self.smoothMotivation()
        }
    }

    /// 置顶 / 取消置顶按钮点击事件
    //: @objc private func topButtonClick() {
    @objc private func positiveAfter() {
    ...
    func everyLast(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
