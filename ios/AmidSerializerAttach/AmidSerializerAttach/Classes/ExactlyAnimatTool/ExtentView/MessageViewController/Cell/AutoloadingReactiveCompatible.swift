...
fileprivate let mDirtText:String = "TSystvoice king product select image"
fileprivate let main_areaId:String = "awake disabledsageCe"
fileprivate let noti_fluentId:String = "useIgreet app safe supposed own"
fileprivate let kSquareIdent:String = "revenue"

/*: "init(coder:) has not been implemented" :*/
fileprivate let main_shareMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_talk_notic" :*/
fileprivate let m_mediaPath:String = "like vessel detail choose classicon_t"
fileprivate let dreamGivenText:String = "finite approachalk_n"
...
// __CLOSE_PRINT__
//
//  AutoloadingReactiveCompatible.swift
//  AbroadTalking
//
...
/**
 * 【模块名称】AutoloadingReactiveCompatible
 * 【功能说明】用于实现聊天窗口中的消息单元。
...
@objc public protocol GatherOperation: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: AutoloadingReactiveCompatible)
    func directBy(cell: AutoloadingReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: AutoloadingReactiveCompatible)
    func promptCell(cell: AutoloadingReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: AutoloadingReactiveCompatible)
    func searchIncome(cell: AutoloadingReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: AutoloadingReactiveCompatible)
    func mpCell(cell: AutoloadingReactiveCompatible)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: AutoloadingReactiveCompatible)
    func minCell(cell: AutoloadingReactiveCompatible)
}

//: @objcMembers public class AutoloadingReactiveCompatible: TCommonTableViewCell {
@objcMembers public class AutoloadingReactiveCompatible: TakeViewCell {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\ReconstructThen.h
     */
    //: public var messageData: ReconstructThen?
    public var messageData: ReconstructThen?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(mDirtText.prefix(5)) + "emMes" + String(main_areaId.suffix(6)) + "ll_Re" + String(noti_fluentId.prefix(4)) + kSquareIdent.replacingOccurrences(of: "revenue", with: "d"))

    /**
     *  协议委托
     *  负责实现 GatherOperation 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: GatherOperation?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_shareMsg.reversed(), encoding: .utf8)!)
    }
    ...
    override public func fieldWith(with data: PoorThen) {
        //: super.fill(with: data)
        super.fieldWith(with: data)
        //: let newData: ReconstructThen = data as! ReconstructThen
        let newData: ReconstructThen = data as! ReconstructThen
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.sizeThinReading()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(avatar(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.imitationClotheStatement(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(spreadOver(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.scribeNumber(name: (String(m_mediaPath.suffix(6)) + String(dreamGivenText.suffix(5)) + "otic"))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(requireGold(recognizer:)))
...
//: extension AutoloadingReactiveCompatible {
extension AutoloadingReactiveCompatible {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func avatar(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.mpCell(cell: self)
    }
