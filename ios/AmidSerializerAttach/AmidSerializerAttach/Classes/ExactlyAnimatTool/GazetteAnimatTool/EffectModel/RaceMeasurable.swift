...
/*: "num" :*/
fileprivate let notiDragSparkId:[Character] = ["n","u","m"]
...
@objc public enum DegreeTransformable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct RaceMeasurable: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// DegreeTransformable
    ...
    mutating func picHistory(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct MultiMeasurable: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(notiDragSparkId))
    }
}
