...
fileprivate let dreamNothingKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "%@.mp4" :*/
fileprivate let main_feeStrikeData:[Character] = ["%","@",".","m","p","4"]

/*: "http" :*/
fileprivate let kRemovalButteProcessData:String = "httstandard"
...
class PubliclyThen: GiftItemView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dreamNothingKey.reversed(), encoding: .utf8)!)
    }
    ...
    override func beyondArc() {
        ...
        let path = ExactlyAnimatTool.shared.outsideFile(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? FindBaseballModelHandyJSON())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(main_feeStrikeData)), path)
...
extension PubliclyThen: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((kRemovalButteProcessData.replacingOccurrences(of: "standard", with: "p"))) {
