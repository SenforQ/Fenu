...
fileprivate let dream_massiveIdent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "iosSvgaEffect" :*/
fileprivate let userAutomaticValue:[Character] = ["i","o","s","S","v","g","a","E","f","f","e"]
fileprivate let k_multiName:[Character] = ["c","t"]

/*: "iosEffect" :*/
fileprivate let showBoardFormat:[Character] = ["i","o","s","E","f","f","e","c","t"]

/*: "entityId" :*/
fileprivate let dreamChallengeName:[Character] = ["e","n","t","i","t","y","I","d"]
...
class FreebeeThen: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: FindBaseballModelHandyJSON?
    ...
    var animatedView: GiftItemView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: OperationStepProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dream_massiveIdent.reversed(), encoding: .utf8)!)
    }
...
extension FreebeeThen {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func noSimultaneously(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(userAutomaticValue) + String(k_multiName))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(showBoardFormat))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(dreamChallengeName))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = during(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func during(msgModel: Any?) -> Bool {
        ...
        if msgModel is FindBaseballModelHandyJSON {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? FindBaseballModelHandyJSON
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is RaceMeasurable {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! RaceMeasurable
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.picHistory(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = FindBaseballModelHandyJSON()
            ...
            effectMsgModel = FindBaseballModelHandyJSON()
            ...
            if giftMsgModel.showType == DegreeTransformable.myStery.rawValue || giftMsgModel.showType == DegreeTransformable.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: FindBaseballModelHandyJSON = self.effectMsgModelArray[i] as! FindBaseballModelHandyJSON
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? FindBaseballModelHandyJSON())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? FindBaseballModelHandyJSON(), at: index)
                }
            ...
            payoffModel(effectMsgModel: effectMsgModel!)
        }
...
extension FreebeeThen {
    //: func pause() {
    func betweenOperation() {
        //: self.paused = true
        self.paused = true
        //: self.animatedView?.pauseAnimation()
        self.animatedView?.birthplace()
    }
    ...
    func stack() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = cameraA() else { return }
                //: firstShow(effectMsgModel: removeModel)
                payoffModel(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.resumeInfoAnimation()
        }
    ...
    func cameraA() -> FindBaseballModelHandyJSON? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return FindBaseballModelHandyJSON()
        }
        ...
        if effectMsgModel is FindBaseballModelHandyJSON {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! FindBaseballModelHandyJSON
        ...
        return effectMsgModel as? FindBaseballModelHandyJSON
    }
}
...
extension FreebeeThen {
    ...
    func payoffModel(effectMsgModel: FindBaseballModelHandyJSON) {
        ...
        self.delegate?.conversionPickObj(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        ExactlyAnimatTool.shared.tag(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.profile(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? GatherHandyJSON())
            //: })
        })
    }
    ...
    func profile(effectMsgModel: FindBaseballModelHandyJSON, success: Bool, effectModel: GatherHandyJSON) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.connect(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    recordingSimultaneously(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func putCustom(effectMsgModel: FindBaseballModelHandyJSON, effectModel: GatherHandyJSON) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.formatAnimating()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.currentAgain()
        ...
        className = ExactlyAnimatTool.shared.load(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! GiftItemView.Type
...
extension FreebeeThen: ReloadReactiveCompatible {
    ...
    func immobilise(effectItemView: GiftItemView, success: Bool) {
        ...
        self.delegate?.viewInsert(effectView: self, obj: effectItemView.effectMsgModel ?? FindBaseballModelHandyJSON(), success: success)
    }
    ...
    func slowDescription(effectItemView: GiftItemView) {
        ...
        self.delegate?.key(effectView: self, obj: effectMsgModel!)
    }
}
...
extension FreebeeThen {
    ...
    func writeData() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.formatAnimating() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.currentAgain() // clean animation data
