...
fileprivate let appCollectionMsg:[UInt8] = [0x50,0x55,0x50,0x5b,0xf,0x4a,0x56,0x4b,0x4c,0x59,0x21,0x10,0x7,0x4f,0x48,0x5a,0x7,0x55,0x56,0x5b,0x7,0x49,0x4c,0x4c,0x55,0x7,0x50,0x54,0x57,0x53,0x4c,0x54,0x4c,0x55,0x5b,0x4c,0x4b]

fileprivate func missArgument(answer num: UInt8) -> UInt8 {
...
/*: "bth_live_follow_pre" :*/
fileprivate let mainWageStr:[Character] = ["b","t","h","_","l","i","v","e","_","f","o","l","l"]
fileprivate let userCustomerMsg:String = "merely cultural enterow_pre"

/*: "btn_live_closed_nor" :*/
fileprivate let m_plotId:[Character] = ["b","t","n","_","l","i","v","e","_","c","l","o","s","e","d","_"]
fileprivate let main_determineStr:String = "noarc"

/*: "removeAttentionUid" :*/
fileprivate let userSumUrl:[Character] = ["r","e","m","o","v","e","A","t","t","e","n","t","i","o","n","U","i"]
fileprivate let mSplitValue:[Character] = ["d"]

/*: "attentionUid" :*/
fileprivate let userSiteText:String = "ATTENT"

/*: "source" :*/
fileprivate let userProposalMessage:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let show_flagTitle:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let dreamUponName:[Character] = ["U","I","D",":"]
...
class NativeTopView: UIView {
    ...
    private var partyModel = FairlyWarnTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appCollectionMsg.map{missArgument(answer: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.scribeNumber(name: (String(mainWageStr) + String(userCustomerMsg.suffix(6)))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(tabSection), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.scribeNumber(name: (String(m_plotId) + main_determineStr.replacingOccurrences(of: "arc", with: "r"))), for: .normal)
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(limitedAcrossRe), for: .touchUpInside)
    ...
    private lazy var onlineListView: LandscapeView = {
...
extension NativeTopView {
    ...
    @objc func noQuantityerest(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(userSumUrl) + String(mSplitValue))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(userSiteText.lowercased() + "ionUid")] = uid
            //: dict["source"] = "2"
            dict[(String(userProposalMessage))] = "2"
        }
...
extension NativeTopView {
    ...
    @objc private func guomindang() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineNum(roomId: partyModel.roomId) { succeed, result, _ in
        OnReqTool.quantity(roomId: partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: self.onlineNum = json["num"].intValue
            self.onlineNum = json[(String(show_flagTitle))].intValue
    ...
    private func describe() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(guomindang), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension NativeTopView {
    ...
    func sendGreetView() {
        ...
        partyModel = LanceManager.doingResource().partyModel
        //: nickNameLab.text = partyModel.streamerInfo.nickname
        nickNameLab.text = partyModel.streamerInfo.nickname
        //: uidLab.text = "UID:\(partyModel.streamerInfo.uid)"
        uidLab.text = (String(dreamUponName)) + "\(partyModel.streamerInfo.uid)"
        ...
        icon.refreshHistory(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.pairControl(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == AdministratorThen.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == AdministratorThen.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            answer(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            answer(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        describe()
    ...
    @objc private func tabSection() {
        //: req_followRequest(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
        noQuantityerest(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.answer(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }
    ...
    @objc private func limitedAcrossRe() {
        //: onlineListView.showView()
        onlineListView.powderStreet()
    }
}
...
extension NativeTopView {
    ...
    private func behindThread() {
        //: frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 43))
        frame = CGRect(x: 0, y: 0, width: noti_senseContent, height: mNameKey + actualWidth(w: 43))
