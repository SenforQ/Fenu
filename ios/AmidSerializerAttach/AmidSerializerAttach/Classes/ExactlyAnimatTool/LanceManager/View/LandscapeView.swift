...
fileprivate let userTingMessage:[UInt8] = [0xb6,0xbb,0xb6,0xc1,0x75,0xb0,0xbc,0xb1,0xb2,0xbf,0x87,0x76,0x6d,0xb5,0xae,0xc0,0x6d,0xbb,0xbc,0xc1,0x6d,0xaf,0xb2,0xb2,0xbb,0x6d,0xb6,0xba,0xbd,0xb9,0xb2,0xba,0xb2,0xbb,0xc1,0xb2,0xb1]

fileprivate func minJaw(resent num: UInt8) -> UInt8 {
...
/*: "Online Users" :*/
fileprivate let app_enableReachUrl:String = "Onlindo pending premium"
...
class LandscapeView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userTingMessage.map{minJaw(resent: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(compute), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(app_enableReachUrl.prefix(5)) + "e Users").localized
        //: return lb
        return lb
        //: }()
    }()
...
extension LandscapeView {
    ...
    private func blunt() {
        ...
        OnReqTool.establish(roomId: LanceManager.doingResource().partyModel.roomId) { succeed, result, _ in
            ...
            if let datas = [MerelyListModel].deserialize(from: array as? Array) {
...
extension LandscapeView {
    ...
    func powderStreet() {
        ...
        contentView.frame = CGRect(x: 0, y: appMineMessage, width: noti_senseContent, height: contentV_H)
        //: reqData()
        blunt()
...
extension LandscapeView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.roundExposure(model: DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension LandscapeView {
    ...
    private func stripFeeConstraint() {
