...
class MarkHasDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: ExactlyNameConvertible?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.tapered()
            }
        }
    ...
    func merely() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.colorTotal(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.landmarkBalance()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.manualColor(), NSAttributedString.Key.font: UIFont.afterDisable(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.colorTotal(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.manualColor(), NSAttributedString.Key.font: UIFont.afterDisable(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.manualColor()
...
extension MarkHasDelegate: UINavigationControllerDelegate {
...
extension MarkHasDelegate: UIGestureRecognizerDelegate {
