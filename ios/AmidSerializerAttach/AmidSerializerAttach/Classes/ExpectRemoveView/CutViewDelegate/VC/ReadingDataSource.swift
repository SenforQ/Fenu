...
/*: "Search" :*/
fileprivate let dreamPlayerStr:String = "color show patient status minSearch"

/*: "#F5F5F5" :*/
fileprivate let show_branchId:String = "#F5F5gage dream"
fileprivate let showWouldFormat:[Character] = ["F","5"]

/*: "Search by UID" :*/
fileprivate let app_sweetPath:[Character] = ["S","e","a","r","c","h"]
fileprivate let dream_memberValue:String = " by UIDauthorize explore"

/*: "icon_search_sea" :*/
fileprivate let user_yesterdaySomeoneMessage:[Character] = ["i","c","o","n","_"]
fileprivate let user_calledBindId:[Character] = ["s","e","a","r"]
fileprivate let dream_selectedIdent:String = "rounding evaluate uniformch_sea"

/*: "Can’t find~" :*/
fileprivate let app_farName:String = "c"
fileprivate let k_hiddenTitle:String = "an’t fcombine stick ta likelihood message"

/*: "icon_kong_kong_default" :*/
fileprivate let user_chemistryId:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n","g","_","d","e","f","a","u"]
fileprivate let showAboutStr:String = "lam"

/*: "Please enter the query UID" :*/
fileprivate let userCompositionUrl:[Character] = ["P","l","e","a","s","e"," ","e","n","t","e","r"," "]
fileprivate let userFirmWageUrl:String = "the qulisten not attribute"
fileprivate let dream_wedIdent:String = "ery UIDauthorize opening female related dismiss"
...
class ReadingDataSource: FlexibleViewController {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(dreamPlayerStr.suffix(6))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.tabOn(colors: UIColor.printColor(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(brightClick), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(show_branchId.prefix(5)) + String(showWouldFormat)))
        //: tf.textColor = UIColor.appTitleColor()
        tf.textColor = UIColor.manualColor()
        //: tf.font = UIFont.pingfangRugularFont(fontSize: 16)
        tf.font = UIFont.soil(fontSize: 16)
        ...
        tf.placeholder = (String(app_sweetPath) + String(dream_memberValue.prefix(7))).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.scribeNumber(name: (String(user_yesterdaySomeoneMessage) + String(user_calledBindId) + String(dream_selectedIdent.suffix(6))))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(ExpectRemoveView.self, forCellReuseIdentifier: ExpectRemoveView.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (app_farName.uppercased() + String(k_hiddenTitle.prefix(6)) + "ind~").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(user_chemistryId) + showAboutStr.replacingOccurrences(of: "am", with: "t"))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + kLocationMessage, width: noti_senseContent, height: appMineMessage - (14 + kLocationMessage + appServerSensePath)), style: style)
    ...
    private lazy var reqManager: MultiSearchManager = //: return MultiSearchManager()
        .init()
    //: }()
}
...
extension ReadingDataSource {
    ...
    private func secondUid() {
        //: self.view.endEditing(true)
        self.view.endEditing(true)
        //: guard searchTF.text?.count ?? 0 > 0 else {
        guard searchTF.text?.count ?? 0 > 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Please enter the query UID".localized)
            self.iconThemeBringBackPlace(showMsg: (String(userCompositionUrl) + String(userFirmWageUrl.prefix(6)) + String(dream_wedIdent.prefix(7))).localized)
            //: return
            return
        }
        ...
        reqManager.stripKeep(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension ReadingDataSource: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: ExpectRemoveView.className(), for: indexPath) as! ExpectRemoveView
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.appearOf(model: model)
...
extension ReadingDataSource: UITextFieldDelegate {
...
extension ReadingDataSource: JXPagingViewListViewDelegate {
