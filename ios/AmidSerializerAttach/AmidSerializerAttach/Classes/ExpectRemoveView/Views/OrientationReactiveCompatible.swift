...
fileprivate let k_addressMsg:[UInt8] = [0xed,0xea,0xed,0xf0,0xac,0xe7,0xeb,0xe0,0xe1,0xf6,0xbe,0xad,0xa4,0xec,0xe5,0xf7,0xa4,0xea,0xeb,0xf0,0xa4,0xe6,0xe1,0xe1,0xea,0xa4,0xed,0xe9,0xf4,0xe8,0xe1,0xe9,0xe1,0xea,0xf0,0xe1,0xe0]

private func exceptContext(ban num: UInt8) -> UInt8 {
    return num ^ 132
}

/*: "bg_medals_podium" :*/
fileprivate let dreamTicketStr:String = "bg_mexplore with rate"
fileprivate let userDepthGeneralIdent:String = "automatic effect lust unknown succeeds_podium"

/*: "icon_rank_first_bg" :*/
fileprivate let mainCertainTopicValue:String = "icviolation"
fileprivate let mainRevenuePhoneIdent:String = "awake end powder this_rank_"
fileprivate let app_foundData:String = "lease alongside writingt_bg"

/*: "icon_rank_first" :*/
fileprivate let appOurselvesMsg:String = "icon_rclock can ago road found"
fileprivate let showMainData:String = "shott"

/*: "icon_ammount_points" :*/
fileprivate let showBalloonStr:String = "modecon"
fileprivate let appUnlessPath:String = "unt_pelectronic sample twenty crop universal"

/*: "icon_lounge" :*/
fileprivate let app_pleaseContent:[Character] = ["i","c","o","n","_"]
fileprivate let noti_conForwardName:String = "loungburn"

/*: "icon_rank_coin" :*/
fileprivate let dreamTrimData:String = "lust tip areaicon_ra"
fileprivate let mArcMessage:String = "nk_coincreate and week study"

/*: "icon_rank_coin2" :*/
fileprivate let m_tailEdgeKey:[Character] = ["i","c","o","n","_","r","a","n"]
fileprivate let noti_centerMoonText:[Character] = ["k","_"]
fileprivate let k_beginningPath:[Character] = ["c","o","i","n","2"]

/*: "icon_rank_second" :*/
fileprivate let mainLayMessage:[Character] = ["i","c","o","n","_","r","a","n","k","_","s","e","c","o","n"]
fileprivate let noti_tradePath:[Character] = ["d"]

/*: "icon_rank_third" :*/
fileprivate let k_psychologicalValue:String = "iheadn"
fileprivate let app_actualPath:String = "holder translate_rank"
...
// __CLOSE_PRINT__
//
//  OrientationReactiveCompatible.swift
//  AbroadTalking
//
...
class OrientationReactiveCompatible: UIView {
    //: var  index = 0
    var index = 0
    ...
    var dataArray: [ShrinkPickHandyJSON] = [] {
        //: didSet {
        didSet {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_addressMsg.map{exceptContext(ban: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.scribeNumber(name: (String(dreamTicketStr.prefix(4)) + "edal" + String(userDepthGeneralIdent.suffix(8))))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = SuiteView(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(faceOperation))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = SuiteView(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = SuiteView(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension OrientationReactiveCompatible {
    //: private func bindInteraction() {
    private func versionFare() {}

    //: private func updateSubviewsWithData() {
    private func commend() {
        //: firstView.updateItemWithModel(model: dataArray[0], type: self.index)
        firstView.ordinary(model: dataArray[0], type: self.index)
        //: secondView.updateItemWithModel(model: dataArray.count>1 ? dataArray[1]: nil, type: self.index)
        secondView.ordinary(model: dataArray.count > 1 ? dataArray[1] : nil, type: self.index)
        //: thirdView.updateItemWithModel(model: dataArray.count>2 ? dataArray[2]: nil, type: self.index)
        thirdView.ordinary(model: dataArray.count > 2 ? dataArray[2] : nil, type: self.index)
    }

    //: @objc func registTapAction(tapGesture: UITapGestureRecognizer) {
    @objc func faceOperation(tapGesture: UITapGestureRecognizer) {
...
extension OrientationReactiveCompatible {
    ...
    private func mentionConstraint() {
        //: let firstWidth = (ScreenWidth - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        let firstWidth = (noti_senseContent - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (noti_senseContent - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
class SuiteView: UIView {
    //: enum SRankHeaderItemType: Int {
    enum ListVarietyEndManHeaderType: Int {
        //: case firstRankType
        case firstRankType
        //: case secondRankType
        case secondRankType
    ...
    public init(frame: CGRect, rankType: ListVarietyEndManHeaderType) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.playMedia(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.scribeNumber(name: (mainCertainTopicValue.replacingOccurrences(of: "violation", with: "on") + String(mainRevenuePhoneIdent.suffix(6)) + "firs" + String(app_foundData.suffix(4))))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.scribeNumber(name: (String(appOurselvesMsg.prefix(6)) + "ank_fir" + showMainData.replacingOccurrences(of: "shot", with: "s")))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .afterDisable(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = WordOfAdviceThen().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.scribeNumber(name: (showBalloonStr.replacingOccurrences(of: "mode", with: "i") + "_ammo" + String(appUnlessPath.prefix(5)) + "oints")), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .afterDisable(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.scribeNumber(name: (String(app_pleaseContent) + noti_conForwardName.replacingOccurrences(of: "burn", with: "e")))
    }
}
...
extension SuiteView {
    ...
    func ordinary(model: ShrinkPickHandyJSON?, type: Int) {
        ...
        headPic.refreshHistory(urlStr: model!.headPic, placeImg: UIImage.pairControl(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.scribeNumber(name: (String(dreamTrimData.suffix(7)) + String(mArcMessage.prefix(7)))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.scribeNumber(name: (String(m_tailEdgeKey) + String(noti_centerMoonText) + String(k_beginningPath))), for: .normal)
        }
    }
}
...
extension SuiteView {
    ...
    func playMedia(type: ListVarietyEndManHeaderType) {
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.scribeNumber(name: (String(appOurselvesMsg.prefix(6)) + "ank_fir" + showMainData.replacingOccurrences(of: "shot", with: "s")))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.fruitThumb(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.scribeNumber(name: (String(mainLayMessage) + String(noti_tradePath)))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.fruitThumb(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.scribeNumber(name: (k_psychologicalValue.replacingOccurrences(of: "head", with: "co") + String(app_actualPath.suffix(5)) + "_third"))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.fruitThumb(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
