...
fileprivate let showSurvivalKey:[UInt8] = [0x8,0xf,0x8,0x15,0x49,0x2,0xe,0x5,0x4,0x13,0x5b,0x48,0x41,0x9,0x0,0x12,0x41,0xf,0xe,0x15,0x41,0x3,0x4,0x4,0xf,0x41,0x8,0xc,0x11,0xd,0x4,0xc,0x4,0xf,0x15,0x4,0x5]

private func wedWarm(slide num: UInt8) -> UInt8 {
    return num ^ 97
}

/*: "icon_rank_coin" :*/
fileprivate let notiHowValue:[UInt8] = [0x6e,0x68,0x74,0x73,0x64,0x77,0x66,0x73,0x70,0x64,0x68,0x74,0x6e,0x73]

fileprivate func versionRecover(plus num: UInt8) -> UInt8 {
    let value = Int(num) - 5
    if value < 0 {
...
/*: "icon_lounge" :*/
fileprivate let notiRiteValue:String = "fiscalc"
fileprivate let noti_spotName:String = "site mark inputon_lounge"

/*: "icon_rank_coin2" :*/
fileprivate let noti_editIdent:String = "iforth"
fileprivate let m_transformTitle:[Character] = ["o","n","_","r","a","n","k","_","c","o","i","n","2"]
...
// __CLOSE_PRINT__
//
//  FairlyReactiveCompatible.swift
//  AbroadTalking
//
...
class FairlyReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showSurvivalKey.map{wedWarm(slide: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.fluent()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .afterDisable(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = WordOfAdviceThen().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.scribeNumber(name: String(bytes: notiHowValue.map{versionRecover(plus: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .afterDisable(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.tabOn(colors: UIColor.printColor(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.scribeNumber(name: (notiRiteValue.replacingOccurrences(of: "fiscal", with: "i") + String(noti_spotName.suffix(9))))
    }
}
...
extension FairlyReactiveCompatible {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func scribe(model: ShrinkPickHandyJSON, index: Int) {
        ...
        headPic.overloadFinish(urlStr: model.headPic)
    ...
    func captainPermission(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: notiHowValue.map{versionRecover(plus: $0)}, encoding: .utf8)! : (noti_editIdent.replacingOccurrences(of: "forth", with: "c") + String(m_transformTitle))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.scribeNumber(name: imageStr), for: .normal)
    }
}
