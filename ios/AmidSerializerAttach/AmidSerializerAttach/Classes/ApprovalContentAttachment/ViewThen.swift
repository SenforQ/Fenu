...
fileprivate let mainCheckUrl:String = "设置\u{666e}通"
fileprivate let k_willingHearUrl:String = "\u{8272}时 请"
fileprivate let mainWithinName:String = "空\u{95f4}的颜色\u{503c}"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let userSlideProposalTitle:[Character] = ["\u{8bbe}","置","选","中","状","态","\u{7684}","文"]
fileprivate let k_bindValue:String = "字\u{989c}色时 请\u{4f7f}"
fileprivate let noti_languageData:[Character] = ["用","R","G","B","空","间","\u{7684}","颜","色","值"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let main_publiclyValue:[UInt8] = [0xcf,0xd4,0xcf,0xda,0x8e,0xc9,0xd5,0xca,0xcb,0xd8,0xa0,0x8f,0x86,0xce,0xc7,0xd9,0x86,0xd4,0xd5,0xda,0x86,0xc8,0xcb,0xcb,0xd4,0x86,0xcf,0xd3,0xd6,0xd2,0xcb,0xd3,0xcb,0xd4,0xda,0xcb,0xca]

fileprivate func moveStat(bring num: UInt8) -> UInt8 {
...
/*: "设置的下标不合法!!" :*/
fileprivate let app_declineStr:String = "设置\u{7684}下标"

/*: "Bags" :*/
fileprivate let k_transformUrl:[Character] = ["B","a","g","s"]
...
open class ViewThen: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: SubsectionPoorSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.boldFill(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((mainCheckUrl + "\u{72b6}态的文字\u{989c}" + k_willingHearUrl + "使用RGB" + mainWithinName))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.boldFill(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(userSlideProposalTitle) + k_bindValue + String(noti_languageData)))
        }
    ...
    fileprivate func boldFill(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: SubsectionPoorSegmentStyle, titles: [String]) {
    ...
    @objc func that(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? FlexibleReactiveCompatible else { return }
        //: currentIndex = currentLabel.tag
        currentIndex = currentLabel.tag
        //: print(currentLabel.tag)
        //: adjustUIWhenBtnOnClickWithAnimate(true)
        aboveAnimaComment(true)
    }

    //: required public init?(coder aDecoder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: main_publiclyValue.map{moveStat(bring: $0)}, encoding: .utf8)!)
    }
}
...
public extension ViewThen {
    ...
    func weekInAnimated(_ selectedIndex: Int, animated: Bool) {
        ...
        aboveAnimaComment(animated)
    }

    // 暴露给外界来刷新标题的显示
    //: public func reloadTitlesWithNewTitles(_ titles: [String]) {
    func panNames(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        weekInAnimated(0, animated: true)
    }
}
...
extension ViewThen {
    ...
    fileprivate func admin() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = FlexibleReactiveCompatible(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.that(_:)))
            ...
            if title == (String(k_transformUrl)).localized {
    ...
    fileprivate func asDisplayPosition() {
        ...
        if let firstLabel = labelsArray[0] as? FlexibleReactiveCompatible {
    ...
    public func clearAdjust(_ currentIndex: Int) {
...
open class FlexibleReactiveCompatible: UILabel {
    /// 用来记录当前label的缩放比例
