...
fileprivate let mainFieldSinkMessage:[UInt8] = [0xf0,0xf7,0xf0,0xed,0xb1,0xfa,0xf6,0xfd,0xfc,0xeb,0xa3,0xb0,0xb9,0xf1,0xf8,0xea,0xb9,0xf7,0xf6,0xed,0xb9,0xfb,0xfc,0xfc,0xf7,0xb9,0xf0,0xf4,0xe9,0xf5,0xfc,0xf4,0xfc,0xf7,0xed,0xfc,0xfd]

/*: "icon_coinxxx" :*/
fileprivate let noti_breadMsg:[Character] = ["i","c","o","n","_","c","o","i","n","x","x"]
fileprivate let userMuseMessage:[Character] = ["x"]

/*: "icon_gift_num_bg" :*/
fileprivate let mTrustMessage:String = "called muse re bronze innericon_"
fileprivate let k_bronzeExhibitData:[Character] = ["_","n","u","m","_","b","g"]
...
class HeapReusableView: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = ApprovalModelType()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainFieldSinkMessage.map{$0^153}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.afterDisable(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: WordOfAdviceThen = {
        //: let btn = TalkingButton.init()
        let btn = WordOfAdviceThen()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.afterDisable(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.scribeNumber(name: (String(noti_breadMsg) + String(userMuseMessage))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.scribeNumber(name: (String(mTrustMessage.suffix(5)) + "gift" + String(k_bronzeExhibitData)))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.afterDisable(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension HeapReusableView {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func betweenAudience(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.fogColor().cgColor
    ...
    func margin(currenmodel: ApprovalModelType, giftType: InjuryConstantTarget) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.overloadFinish(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.overloadFinish(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.afterDisable(type: .Regular, fontSize: 11),
...
extension HeapReusableView {
    ...
    func applySession() {
        ...
        bgView.backgroundColor = UIColor.stopExit()
