...
/*: "&sr=6" :*/
fileprivate let notiCreationPath:String = "reply on title&sr=6"
...
enum AttemptNameLiteral: String, CaseIterable {
...
enum ConcentrationTextLiteral: String {
...
class CcrcReactiveCompatible: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func inquireAsset(urlType: ConcentrationTextLiteral) -> String {
        //: var url = AdministratorThen.share.appConfigMode.urlH5Domain
        var url = AdministratorThen.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(AdministratorThen.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : AdministratorThen.share.appUserConfigMode.fullRechargePage)
            url.append(AdministratorThen.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : AdministratorThen.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(AdministratorThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : AdministratorThen.share.appUserConfigMode.halfRechargePage)
            url.append(AdministratorThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : AdministratorThen.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = AdministratorThen.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = AdministratorThen.share.appUserConfigMode.halfRechargePage.appending((String(notiCreationPath.suffix(5))))
            //: url.append(AdministratorThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(AdministratorThen.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
