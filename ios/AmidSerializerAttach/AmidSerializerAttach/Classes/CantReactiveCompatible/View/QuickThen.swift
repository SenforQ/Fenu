...
fileprivate let show_videoMsg:[UInt8] = [0x9e,0x99,0x9e,0x83,0xdf,0x94,0x98,0x93,0x92,0x85,0xcd,0xde,0xd7,0x9f,0x96,0x84,0xd7,0x99,0x98,0x83,0xd7,0x95,0x92,0x92,0x99,0xd7,0x9e,0x9a,0x87,0x9b,0x92,0x9a,0x92,0x99,0x83,0x92,0x93]

private func dailyChild(quantity num: UInt8) -> UInt8 {
    return num ^ 247
}

/*: "8075F5" :*/
fileprivate let m_originButtonKey:String = "8075F5cookie attach argument"

/*: "9A9B9C" :*/
fileprivate let dreamAddressMessage:String = "restriction trim receive tell9A9B9C"

/*: "kOpacityAnimation" :*/
fileprivate let main_zzName:[Character] = ["k","O","p","a","c","i","t","y","A","n","i","m","a","t"]
fileprivate let m_feministData:String = "imagazine"

/*: "opacity" :*/
fileprivate let app_failUrl:[Character] = ["o","p","a","c","i","t","y"]

/*: "forwards" :*/
fileprivate let main_expressText:String = "bossrwar"
fileprivate let mHoldMsg:[Character] = ["d","s"]

/*: "easeIn" :*/
fileprivate let show_resFormat:[Character] = ["e","a","s","e","I"]
fileprivate let dream_brandMsg:String = "min"

/*: "8C7AF8" :*/
fileprivate let k_livingId:[Character] = ["8","C","7","A","F","8"]
...
class QuickThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_videoMsg.map{dailyChild(quantity: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(fare(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension QuickThen {
    ...
    func indicator() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(m_originButtonKey.prefix(6))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(dreamAddressMessage.suffix(6))))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func skin(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(main_zzName) + m_feministData.replacingOccurrences(of: "magazine", with: "on")))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(range(), forKey: (String(main_zzName) + m_feministData.replacingOccurrences(of: "magazine", with: "on")))
        }
    ...
    fileprivate func range() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (String(app_failUrl)))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (main_expressText.replacingOccurrences(of: "boss", with: "fo") + String(mHoldMsg)))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(show_resFormat) + dream_brandMsg.replacingOccurrences(of: "min", with: "n"))))
        //: return animation
        return animation
    }
}
...
extension QuickThen {
    ...
    private func minorSubviews() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(k_livingId)))?.cgColor
