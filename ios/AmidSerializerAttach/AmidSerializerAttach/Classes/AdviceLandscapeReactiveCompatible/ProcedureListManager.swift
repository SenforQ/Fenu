...
/*: "uid" :*/
fileprivate let user_straightKey:String = "uistrike"

/*: "onlineStatus" :*/
fileprivate let notiCirclePath:String = "onlinresolution"

/*: "isNewUser" :*/
fileprivate let showDutyContent:String = "magnitude pose ego pastisNew"
fileprivate let mainAllowFloorStr:[Character] = ["U","s","e","r"]

/*: "isTPAuth" :*/
fileprivate let dreamHealthyWarmUrl:[Character] = ["i","s","T","P","A","u","t","h"]

/*: "totalIntimate" :*/
fileprivate let showNowUrl:[Character] = ["t","o","t","a","l"]
fileprivate let main_harassmentContent:[Character] = ["I"]
fileprivate let noti_ofFatalId:[Character] = ["n","t","i","m","a","t","e"]

/*: "userStatus" :*/
fileprivate let mOurselvesFormat:[Character] = ["u","s","e","r","S"]
fileprivate let userNarrowTitle:[Character] = ["t","a","t","u","s"]

/*: "intimate" :*/
fileprivate let main_nurseName:[Character] = ["i","n","t","i","m"]
fileprivate let app_conversionContent:String = "ATE"
...
enum ProblemHeapOffsetTarget: Int {
    //: case offical  = 0
    case offical = 0 // 官方
    //: case chatRoom = 1
    case chatRoom = 1 // 聊天室
...
enum DeletePropertyProtocol: Int {
    //: case top  = 0
    case top = 0 // 置顶
    //: case normal = 1
    case normal = 1 // 默认
...
class ProcedureListManager {
    ...
    private var moreMsgModel: ModelThen = //: return ModelThen.getPlaceholderModel(chatType: .moreMsg)
        .moment(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func secureData() {
        //: self.officialArr = EdgeMsgListener.shared.getOfficialList()
        self.officialArr = EdgeMsgListener.shared.serration()
        //: self.topArr = EdgeMsgListener.shared.topConvList
        self.topArr = EdgeMsgListener.shared.topConvList
        //: self.normalArr = EdgeMsgListener.shared.norConvList
        self.normalArr = EdgeMsgListener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        commitSection()
    ...
    func attending(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: EdgeMsgListener.shared.getChatRoomData { succeed in
        EdgeMsgListener.shared.statReject { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = EdgeMsgListener.shared.chatRoomList
                self.chatRoomArr = EdgeMsgListener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func second(indexPath: IndexPath) -> ModelThen? {
    ...
    func grade(model: ModelThen?) -> ModelThen? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: MerelyMeasurable? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: MerelyMeasurable? = DecisionMakerReactiveCompatible.clout(targetId: targetId)
    ...
    func openPriority(conversationList: [ModelThen], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        DecisionMakerReactiveCompatible.with(with: tempArr) { _, error in
    ...
    func way(conversationModel: ModelThen) {
    ...
    func lastFor(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: EdgeMsgListener.shared.func__getConversationList { arr, leftMore in
        EdgeMsgListener.shared.us { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [ModelThen] = //: return Array<ModelThen>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [ModelThen] = //: return Array<ModelThen>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [ModelThen] = //: return Array<ModelThen>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [ModelThen] = //: return Array<ModelThen>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [ModelThen] = //: return Array<ModelThen>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [ModelThen] = //: return Array<ModelThen>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [ModelThen] = //: return Array<ModelThen>()
        .init()
    //: }()
}
...
extension ProcedureListManager {
    ...
    func commitSection() {
        //: self.moreMsgArr = EdgeMsgListener.shared.moreMsgList
        self.moreMsgArr = EdgeMsgListener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func diskBy(indexPath: IndexPath) -> ModelThen? {
    ...
    func atContainModel() -> ModelThen {
        ...
        moreMsgModel.unreadCount = cornerDisplay()
...
extension ProcedureListManager {
    ...
    func dialogOperation(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        DecisionMakerReactiveCompatible.portionCalled(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(user_straightKey.replacingOccurrences(of: "strike", with: "d"))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(notiCirclePath.replacingOccurrences(of: "resolution", with: "e") + "Status")].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(showDutyContent.suffix(5)) + String(mainAllowFloorStr))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(dreamHealthyWarmUrl))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(showNowUrl) + String(main_harassmentContent) + String(noti_ofFatalId))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(mOurselvesFormat) + String(userNarrowTitle))].int ?? 1
                    ...
                    self.soul(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func modifyUponArcanum(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(user_straightKey.replacingOccurrences(of: "strike", with: "d"))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(notiCirclePath.replacingOccurrences(of: "resolution", with: "e") + "Status")] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(showDutyContent.suffix(5)) + String(mainAllowFloorStr))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(mOurselvesFormat) + String(userNarrowTitle))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(mOurselvesFormat) + String(userNarrowTitle))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension ProcedureListManager {
    ...
    func manager() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in EdgeMsgListener.shared.norConvList {
        for model in EdgeMsgListener.shared.norConvList {
            //: if model.totalIntimate >= AdministratorThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= AdministratorThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.grade(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in EdgeMsgListener.shared.topConvList {
        for model in EdgeMsgListener.shared.topConvList {
            //: if model.totalIntimate >= AdministratorThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= AdministratorThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func pinContext(convModel: ModelThen, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func psychologicalMoment(indexPath: IndexPath) -> ModelThen? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func soul(uid: String, intimate: Int) {
        ...
        DecisionMakerReactiveCompatible.convertTime(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func permissionKey(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(user_straightKey.replacingOccurrences(of: "strike", with: "d"))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(String(main_nurseName) + app_conversionContent.lowercased())] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
