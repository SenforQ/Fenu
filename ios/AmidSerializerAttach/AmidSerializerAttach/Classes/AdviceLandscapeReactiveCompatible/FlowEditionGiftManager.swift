...
/*: "gifts" :*/
fileprivate let noti_kitName:String = "GIFTS"

/*: "vipGifts" :*/
fileprivate let app_springValue:String = "communicate outcome discount combine boavipGi"
fileprivate let show_bassFormat:[Character] = ["f","t","s"]

/*: "packGifts" :*/
fileprivate let main_shotMsg:String = "pawindk"

/*: "giftSendItem" :*/
fileprivate let dreamDigitalCapableName:[Character] = ["g","i","f","t","S","e"]
fileprivate let user_providerFormat:String = "rating minutendItem"
...
class FlowEditionGiftManager: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [InjuryConstantTarget: [Any]]]()
    // 数据锁
    ...
    private var giftType = AdviceExpressionConvertible.normal

    //: static let share = TalkingChatGiftManager()
    static let share = FlowEditionGiftManager()
...
extension FlowEditionGiftManager {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func prefaceCompletion(type: AdviceExpressionConvertible = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        FlowEditionGiftManager.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[InjuryConstantTarget.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            FlowEditionGiftManager.share.jaw(completion: { isSucceed in
            ...
            FlowEditionGiftManager.share.ii(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            FlowEditionGiftManager.share.eventReduce(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            FlowEditionGiftManager.share.shapeSpring(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func shapeSpring(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = FlowEditionGiftManager.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        ShrinkReactiveCompatible.picPriority(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [InjuryConstantTarget: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(noti_kitName.lowercased())] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ApprovalModelType>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[InjuryConstantTarget.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(app_springValue.suffix(5)) + String(show_bassFormat))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ApprovalModelType>.deserialize(from: array as? Array) {
                    ...
                    dataDict[InjuryConstantTarget.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(main_shotMsg.replacingOccurrences(of: "wind", with: "c") + "Gifts")] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ApprovalModelType>.deserialize(from: array as? Array) {
                    ...
                    dataDict[InjuryConstantTarget.Bags] = bagsArr
                }
                ...
                var firstModel = SprinklerSystemTransformable()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(dreamDigitalCapableName) + String(user_providerFormat.suffix(6)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<SprinklerSystemTransformable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func jaw(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = FlowEditionGiftManager.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        ShrinkReactiveCompatible.fictitiousCharacter(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func sodalistBefore(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        ShrinkReactiveCompatible.sceneCompletion(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<LanceTransformable>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func ii(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = AdviceExpressionConvertible.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        ShrinkReactiveCompatible.transport(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func eventReduce(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = AdviceExpressionConvertible.party
        //: TalkingChatRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        ShrinkReactiveCompatible.transitionCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func nature(perPageCount: Int, dataType: InjuryConstantTarget) -> Int {
    ...
    func curResistance(dataType: InjuryConstantTarget) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func draftItems() -> [Any] {
        ...
        if FlowEditionGiftManager.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if FlowEditionGiftManager.share.giftType == .live || FlowEditionGiftManager.share.giftType == .party {
    ...
    func beginning(dataType: InjuryConstantTarget, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension FlowEditionGiftManager {
    ...
    private func transactionAcross() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(currentBot),
    ...
    @objc private func currentBot() {
        //: gift_clearMemoryCache(showRed: true)
        clearRed(showRed: true)
    }
}
...
extension FlowEditionGiftManager {
    ...
    func clearRed(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        FlowEditionGiftManager.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
