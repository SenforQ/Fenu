...
fileprivate let notiInviteId:[UInt8] = [0x48,0x4d,0x48,0x53,0x7,0x42,0x4e,0x43,0x44,0x51,0x19,0x8,0xff,0x47,0x40,0x52,0xff,0x4d,0x4e,0x53,0xff,0x41,0x44,0x44,0x4d,0xff,0x48,0x4c,0x4f,0x4b,0x44,0x4c,0x44,0x4d,0x53,0x44,0x43]

fileprivate func portionLovely(week num: UInt8) -> UInt8 {
...
/*: "SHBDropMenuCell" :*/
fileprivate let dreamSimultaneouslyTitle:[Character] = ["S","H","B","D","r","o","p","M","e","n"]
fileprivate let showAdjustmentMsg:String = "uCelloperation resign"

/*: "title" :*/
fileprivate let dream_comprehensiveTitle:String = "titbirthdaye"
...
public class MenuReactiveCompatible: UIView {
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.CollectionConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.MenuConfig.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.MenuConfig.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.MenuConfig.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? SeeMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func asStar(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> MenuReactiveCompatible {
        ...
        let menuView = asValue(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func asValue(anchorView: UIView, menuArray: [CounterdependMenuModel]) -> MenuReactiveCompatible {
        //: let window = CreditsThen.getWindow()
        let window = CreditsThen.captain()
        //: let menuView = DropDownMenuView()
        let menuView = MenuReactiveCompatible()
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiInviteId.map{portionLovely(week: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func awakeToView() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func viewForArrow() -> UITableView {
    ...
    private func anyBasic() {
        //: addSubview(contentView)
        addSubview(contentView)
        //: mTable.register(DropMenuCell.self, forCellReuseIdentifier: "SHBDropMenuCell")
        mTable.register(CompartmentMenuRemoveThen.self, forCellReuseIdentifier: (String(dreamSimultaneouslyTitle) + String(showAdjustmentMsg.prefix(5))))
        //: contentView.addSubview(mTable)
        contentView.addSubview(mTable)
    }

    //: private func refreshUI() {
    private func whiteResistance() {
        //: drawmTableFrame()
        column()
        //: mTable.reloadData()
        mTable.reloadData()
    ...
    private func counterval() -> CGFloat {
...
extension MenuReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(dreamSimultaneouslyTitle) + String(showAdjustmentMsg.prefix(5))), for: indexPath) as! CompartmentMenuRemoveThen
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (dream_comprehensiveTitle.replacingOccurrences(of: "birthday", with: "l")))
        //: animateRemoveView()
        eraseAnimateView()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.input(index: indexPath.row, title: cellModel.title ?? "")
    }
}
