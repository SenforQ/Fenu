...
fileprivate let mBottomFormat:[UInt8] = [0xc6,0xc1,0xc6,0xdb,0x87,0xcc,0xc0,0xcb,0xca,0xdd,0x95,0x86,0x8f,0xc7,0xce,0xdc,0x8f,0xc1,0xc0,0xdb,0x8f,0xcd,0xca,0xca,0xc1,0x8f,0xc6,0xc2,0xdf,0xc3,0xca,0xc2,0xca,0xc1,0xdb,0xca,0xcb]

private func workerRecognize(pep num: UInt8) -> UInt8 {
    return num ^ 175
}

/*: "Edit Title" :*/
fileprivate let appAppealMsg:String = "Edit Trow mode"

/*: "Finish" :*/
fileprivate let app_seatMessage:[Character] = ["F","i","n","i","s","h"]

/*: "btn_delete" :*/
fileprivate let appHemGivingFormat:String = "btn_dhundred count carrier abs bring"
fileprivate let mMotivationIdent:[Character] = ["e"]
...
class EnablelyTextView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(tarPick))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mBottomFormat.map{workerRecognize(pep: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(appAppealMsg.prefix(6)) + "itle").localized
    ...
    lazy var inputTView: TwoHasDelegate = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(app_seatMessage)).localized, for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.scribeNumber(name: (String(appHemGivingFormat.prefix(5)) + "elet" + String(mMotivationIdent))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(productThrough), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension EnablelyTextView: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = invite(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func invite(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
