...
fileprivate let dream_distinguishUrl:[UInt8] = [0x42,0x45,0x42,0x5f,0x3,0x48,0x44,0x4f,0x4e,0x59,0x11,0x2,0xb,0x43,0x4a,0x58,0xb,0x45,0x44,0x5f,0xb,0x49,0x4e,0x4e,0x45,0xb,0x42,0x46,0x5b,0x47,0x4e,0x46,0x4e,0x45,0x5f,0x4e,0x4f]

private func boxVid(zone num: UInt8) -> UInt8 {
    return num ^ 43
}

/*: "Add more details of yourself" :*/
fileprivate let noti_globalFlushFormat:[Character] = ["A","d","d"," ","m","o","r","e"," ","d","e","t","a","i","l","s"," ","o","f"," "]
fileprivate let showAppealStr:[Character] = ["y","o"]
fileprivate let appIonFormat:String = "monthself"

/*: "#999999" :*/
fileprivate let m_cheekValue:[Character] = ["#","9","9","9","9","9","9"]

/*: "0/500" :*/
fileprivate let dreamCommandStr:String = "0/500"

/*: "Tip:Don't post content that induces others to send gifts or other money-relatsed content." :*/
fileprivate let m_ehStr:[UInt8] = [0x2e,0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63,0x20,0x64,0x65,0x73,0x74,0x61,0x6c,0x65,0x72,0x2d,0x79,0x65,0x6e,0x6f,0x6d,0x20,0x72,0x65,0x68,0x74,0x6f,0x20,0x72,0x6f,0x20,0x73,0x74,0x66,0x69,0x67,0x20,0x64,0x6e,0x65,0x73,0x20,0x6f,0x74,0x20,0x73,0x72,0x65,0x68,0x74,0x6f,0x20,0x73,0x65,0x63,0x75,0x64,0x6e,0x69,0x20,0x74,0x61,0x68,0x74,0x20,0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63,0x20,0x74,0x73,0x6f,0x70,0x20,0x74,0x27,0x6e,0x6f,0x44,0x3a,0x70,0x69,0x54]
...
class ReadingFindView: UITableViewCell {
    //: let limitCount = 500
    let limitCount = 500
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(aboutNotification(notification:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(anonymous(notification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dream_distinguishUrl.map{boxVid(zone: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(noti_globalFlushFormat) + String(showAppealStr) + appIonFormat.replacingOccurrences(of: "month", with: "ur")).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var numberLB: UILabel = {
        ...
        LB.textColor = UIColor(hex: (String(m_cheekValue)))
        //: LB.font = UIFont.pingfangFont(type: .Regular, fontSize: 14)
        LB.font = UIFont.afterDisable(type: .Regular, fontSize: 14)
        //: LB.text = "0/500"
        LB.text = (dreamCommandStr.capitalized)
    ...
    lazy var textView: TwoHasDelegate = {
        //: let textView = TalkingTextView()
        let textView = TwoHasDelegate()
        //: let sign = AdministratorThen.share.loginUserMode.signature
        let sign = AdministratorThen.share.loginUserMode.signature
        //: textView.text = sign
        textView.text = sign
        //: textView.placeholder = "Tip:Don't post content that induces others to send gifts or other money-relatsed content.".localized
        textView.placeholder = String(bytes: m_ehStr.reversed(), encoding: .utf8)!.localized
        //: textView.placeholderColor = UIColor.appValueDetailColor()
        textView.placeholderColor = UIColor.fluent()
        ...
        textView.font = UIFont.afterDisable(type: .Regular, fontSize: 15)
        ...
        textView.backgroundColor = .landmarkBalance()
...
extension ReadingFindView {
    //: @objc func keyboardWillBeHidden(notification: NSNotification) {
    @objc func aboutNotification(notification _: NSNotification) {
    ...
    @objc func anonymous(notification: NSNotification) {
...
extension ReadingFindView: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > limitCount {
            ...
            if textNum > limitCount && limitCount > 0 {
                ...
                textView.text = productInside(index: limitCount, text: textContent)
            }
        }
        //: numberLB.text =  "\(textView.text.count)/\(limitCount)"
        numberLB.text = "\(textView.text.count)/\(limitCount)"
    ...
    private func productInside(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
