...
fileprivate let show_possibleStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#333333" :*/
fileprivate let notiPutPath:[Character] = ["#","3","3","3","3","3","3"]

/*: "My photos" :*/
fileprivate let main_observeTitle:[Character] = ["M","y"]
fileprivate let main_occurName:[Character] = [" ","p","h","o","t","o","s"]

/*: "#999999" :*/
fileprivate let dream_someoneIdent:[Character] = ["#","9","9","9","9","9","9"]

/*: "Click to change or delete photo." :*/
fileprivate let showBackgroundIdent:[UInt8] = [0xb0,0x9f,0x9a,0x90,0x98,0xd3,0x87,0x9c,0xd3,0x90,0x9b,0x92,0x9d,0x94,0x96,0xd3,0x9c,0x81,0xd3,0x97,0x96,0x9f,0x96,0x87,0x96,0xd3,0x83,0x9b,0x9c,0x87,0x9c,0xdd]

private func lieActive(robot num: UInt8) -> UInt8 {
    return num ^ 243
}

/*: "btn_me_edit_photo_add_nor" :*/
fileprivate let mSomeoneFormat:String = "feminist instancebtn_m"
fileprivate let noti_annualPath:String = "oval year_photo"

/*: "btn_me_edit_photo_add_pre" :*/
fileprivate let noti_grayMessage:[Character] = ["b","t","n","_","m","e","_","e","d"]
fileprivate let m_perRemMsg:String = "spot menu nationalit_pho"
fileprivate let m_sunMsg:String = "rocket paper trimd_pre"

/*: "btn_me_edit_photo_delete" :*/
fileprivate let notiAbsListenerPath:String = "btn_mneed plus past merge"
fileprivate let notiArrowUnableStr:String = "e_editprotocol pull"
fileprivate let kConstraintKey:String = "o_desixth composition examine"
...
public protocol EditionReactiveCompatible: NSObject {
    /// 图片add
...
class MightReactiveCompatible: UITableViewCell {
    //: let disposeBag = DisposeBag()
    let disposeBag = DisposeBag()
    //: var count = 0
    var count = 0
    //: open weak var delegate: EditPhotoDelegate?
    open weak var delegate: EditionReactiveCompatible?
...
//        showMessageView()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_possibleStr.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (String(notiPutPath)))
        //: lb.text = "My photos".localized
        lb.text = (String(main_observeTitle) + String(main_occurName)).localized
    ...
    lazy var messageLB: UILabel = {
        ...
        lb.font = UIFont.afterDisable(type: .Regular, fontSize: 12)
        //: lb.textColor = UIColor.init(hex: "#999999")
        lb.textColor = UIColor(hex: (String(dream_someoneIdent)))
        //: lb.text = "Click to change or delete photo.".localized
        lb.text = String(bytes: showBackgroundIdent.map{lieActive(robot: $0)}, encoding: .utf8)!.localized
        //: return lb
        return lb
        //: }()
    }()
...
extension MightReactiveCompatible {
    ...
    func wantView() {
        ...
        for i in 0 ..< 9 {
            ...
            colorBtn.setBackgroundImage(.scribeNumber(name: (String(mSomeoneFormat.suffix(5)) + "e_edit" + String(noti_annualPath.suffix(6)) + "_add_nor")), for: .normal)
            //: colorBtn.setBackgroundImage(.BundleImageNamed(name: "btn_me_edit_photo_add_pre"), for: .highlighted)
            colorBtn.setBackgroundImage(.scribeNumber(name: (String(noti_grayMessage) + String(m_perRemMsg.suffix(6)) + "to_ad" + String(m_sunMsg.suffix(5)))), for: .highlighted)
            ...
            deleteBtn.setBackgroundImage(.scribeNumber(name: (String(notiAbsListenerPath.prefix(5)) + String(notiArrowUnableStr.prefix(6)) + "_phot" + String(kConstraintKey.prefix(4)) + "lete")), for: .normal)
    ...
    @objc func master(sender: UIButton) {
        //: var isReturn = false
        var isReturn = false
        ...
        AsidePermissionTool.replaceDown(true) { (isOpen: Bool) in
            ...
            if isOpen {
                ...
                let vc = GagePickTool.examine(maxCount: 9 - self.count, allowPhoto: true, allowVideo: false)
