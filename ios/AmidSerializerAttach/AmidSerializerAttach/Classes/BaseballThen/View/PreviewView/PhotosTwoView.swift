...
fileprivate let main_mightData:[UInt8] = [0x8a,0x8d,0x8a,0x97,0xcb,0x80,0x8c,0x87,0x86,0x91,0xd9,0xca,0xc3,0x8b,0x82,0x90,0xc3,0x8d,0x8c,0x97,0xc3,0x81,0x86,0x86,0x8d,0xc3,0x8a,0x8e,0x93,0x8f,0x86,0x8e,0x86,0x8d,0x97,0x86,0x87]

/*: "img_loading_lose" :*/
fileprivate let dream_draftFlamePath:String = "img_lobelow calculation build"
fileprivate let mainStickLaunchMsg:String = "g_lored future witch feedback remain"
fileprivate let k_femaleDonName:String = "sdescription"
...
class PhotosTwoView: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        self.destinationNumbereraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: main_mightData.map{$0^227}, encoding: .utf8)!)
    }
...
extension PhotosTwoView {
    //: private func bindInteraction() {
    private func destinationNumbereraction() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(outAppearance))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(regularise)).then {
...
extension PhotosTwoView {
    ...
    func recommendNetwork(imgModel: JawHandyJSON) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(dream_draftFlamePath.prefix(6)) + "adin" + String(mainStickLaunchMsg.prefix(4)) + k_femaleDonName.replacingOccurrences(of: "description", with: "e"))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.recoverRound()
            }
        }
    ...
    func viaWith(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(dream_draftFlamePath.prefix(6)) + "adin" + String(mainStickLaunchMsg.prefix(4)) + k_femaleDonName.replacingOccurrences(of: "description", with: "e"))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.recoverRound()
        }
    }
...
extension PhotosTwoView: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension PhotosTwoView {
    ...
    func recoverRound() {
        ...
        var newW = noti_senseContent
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = noti_senseContent * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > appMineMessage {
            //: newH = ScreenHeight
            newH = appMineMessage
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * appMineMessage / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (noti_senseContent - newW) / 2.0, y: (appMineMessage - newH) / 2)
