...
fileprivate let notiWhetherFormat:String = "currently yaicon_"
fileprivate let mShirtFormat:String = "kong_behind request again"
fileprivate let noti_reduceButtonMsg:String = "defcameralt"

/*: "page" :*/
fileprivate let appLeaveValue:[UInt8] = [0x4d,0x3e,0x44,0x42]

fileprivate func imageDisplayDecline(plus num: UInt8) -> UInt8 {
...
/*: "uid" :*/
fileprivate let appLabelKey:[UInt8] = [0x2c,0x30,0x3d]

/*: "list" :*/
fileprivate let show_decentName:String = "lmomentst"
...
class MerelyViewController: FlexibleViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [CantItemModel]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: userRouteUrl, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(ExactlyReusableView.self, forCellWithReuseIdentifier: ExactlyReusableView.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.scribeNumber(name: (String(notiWhetherFormat.suffix(5)) + "kong_" + String(mShirtFormat.prefix(5)) + noti_reduceButtonMsg.replacingOccurrences(of: "camera", with: "au")))
...
extension MerelyViewController {
    ...
    private func reevaluationData() {
        //: let params = ["page": pageIndex, "uid": userInfoModel.uid] as [String: Any]
        let params = [String(bytes: appLeaveValue.map{imageDisplayDecline(plus: $0)}, encoding: .utf8)!: pageIndex, String(bytes: appLabelKey.map{$0^89}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        LiteralReactiveCompatible.post(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(show_decentName.replacingOccurrences(of: "moment", with: "i"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = LogHandyJSON.deserialize(from: dict as? Dictionary) {
...
extension MerelyViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ExactlyReusableView.className(), for: indexPath) as! ExactlyReusableView
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = DetailFlushViewDelegate(model: detailModel)
...
extension MerelyViewController: JXPagingViewListViewDelegate {
...
extension MerelyViewController {
    ...
    func ditTo(_ userModel: GatherTransformable) {
