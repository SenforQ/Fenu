...
fileprivate let mainReduceFormat:[Character] = ["U","s","e","r","D","e","t","a","i","l","T","a","g","M","e","C","e"]
fileprivate let dreamCleanIdentityName:[Character] = ["l","l","I","D"]

/*: "UserDetailTagInterestCellID" :*/
fileprivate let mainPsychologicalName:String = "kit captainUser"
fileprivate let showActualRapidlyId:String = "route messagelTagIn"
fileprivate let userDutyIdent:[Character] = ["l","I","D"]

/*: "AIHobbyCellID" :*/
fileprivate let kGrayMessage:[Character] = ["A","I","H","o","b"]
fileprivate let mAgencyStr:[Character] = ["b","y","C","e","l","l","I","D"]
...
class AccountObjectProtocol: FlexibleViewController {
    ...
    private var tableHeightModel = BonfireThen() // 记录分区高度
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: userRouteUrl, right: 0)
        ...
        tableView.register(SampleReactiveCompatible.self, forCellReuseIdentifier: SampleReactiveCompatible.className())
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "UserDetailTagMeCellID")
        tableView.register(PapersReactiveCompatible.self, forCellReuseIdentifier: (String(mainReduceFormat) + String(dreamCleanIdentityName)))
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "UserDetailTagInterestCellID")
        tableView.register(PapersReactiveCompatible.self, forCellReuseIdentifier: (String(mainPsychologicalName.suffix(4)) + "Detai" + String(showActualRapidlyId.suffix(6)) + "terestCel" + String(userDutyIdent)))
        //: tableView.register(TalkingUserDetailGiftCell.self, forCellReuseIdentifier: TalkingUserDetailGiftCell.className())
        tableView.register(TowardViewCell.self, forCellReuseIdentifier: TowardViewCell.className())
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "AIHobbyCellID")
        tableView.register(PapersReactiveCompatible.self, forCellReuseIdentifier: (String(kGrayMessage) + String(mAgencyStr)))
        //: return tableView
        return tableView
        //: }()
    }()
...
extension AccountObjectProtocol: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellType: CreateCellType = self.sectionArr[indexPath.section]
        //: switch cellType {
        switch cellType {
        //: case .profileType:
        case .profileType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: SampleReactiveCompatible.className(), for: indexPath) as! SampleReactiveCompatible
            //: cell.configWithDataModel(userModel: userInfoModel)
            cell.enterModel(userModel: userInfoModel)
            //: self.setProfileCellHeight(signAttribute: cell.signLab.attributedText ?? NSAttributedString.init(string: ""))
            self.answerAttribute(signAttribute: cell.signLab.attributedText ?? NSAttributedString(string: ""))
            //: return cell
            return cell
        ...
        case .aboutMeType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: (String(mainReduceFormat) + String(dreamCleanIdentityName)), for: indexPath) as! PapersReactiveCompatible
            //: cell.configTagCellWithDataModel(type: TUserDetailCellType.aboutMeType, userModel: userInfoModel)
            cell.underestimate(type: CreateCellType.aboutMeType, userModel: userInfoModel)
        ...
        case .intersetsType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: (String(mainPsychologicalName.suffix(4)) + "Detai" + String(showActualRapidlyId.suffix(6)) + "terestCel" + String(userDutyIdent)), for: indexPath) as! PapersReactiveCompatible
            //: cell.configTagCellWithDataModel(type: TUserDetailCellType.intersetsType, userModel: userInfoModel)
            cell.underestimate(type: CreateCellType.intersetsType, userModel: userInfoModel)
            //: cell.collectionLayoutSubviewBlock = {[weak self] collectH in
            cell.collectionLayoutSubviewBlock = { [weak self] collectH in
                //: guard let self = self else { return }
                guard let self = self else { return }
        ...
        case .giftType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: TowardViewCell.className(), for: indexPath) as! TowardViewCell
...
extension AccountObjectProtocol {
    //: private func setProfileCellHeight(signAttribute: NSAttributedString) {
    private func answerAttribute(signAttribute: NSAttributedString) {
        ...
        var labelH = signAttribute.boundingRect(with: CGSize(width: noti_senseContent - 58, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil).size.height
    ...
    private func justAbout() -> CGFloat {
...
extension AccountObjectProtocol: JXPagingViewListViewDelegate {
...
extension AccountObjectProtocol {
    ...
    func verticalProfile(_ giftArr: [LogMeasurable],
                         //: _ sectionArr: [TUserDetailCellType],
                         _ sectionArr: [CreateCellType],
                         //: _ userModel: TalkingUserInfoModel) {
                         _ userModel: GatherTransformable)
    {
