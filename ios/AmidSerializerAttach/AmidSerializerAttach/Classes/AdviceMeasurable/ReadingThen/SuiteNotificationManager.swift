...
/*: "rYMsgType" :*/
fileprivate let m_riteUrl:String = "rYMsgTfinancial engage pad"
fileprivate let kModeStr:String = "ytransform"

/*: "GJ:CallCustom" :*/
fileprivate let mainSometimeValue:[Character] = ["G","J",":","C","a","l","l","C"]
fileprivate let kDepthMsg:[Character] = ["u","s","t","o","m"]
...
// __CLOSE_PRINT__
//
//  SuiteNotificationManager.swift
//  AbroadTalking
//
...
class SuiteNotificationManager: NSObject {
    //: private var showView: TalkingMessageNotificationView?
    private var showView: ReadingThen? // 当前View
    //: private var preView: TalkingMessageNotificationView?
    private var preView: ReadingThen? // 旧的View
    ...
    static var shared = SuiteNotificationManager()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(clearCutAggregation),
    ...
    private lazy var msgArr: [ModelThen] = //: return [ModelThen]()
        .init()
    //: }()
}
...
extension SuiteNotificationManager {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func oppositeness(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard keepOut() == true || image() == true else { return }
        //: guard msg.userID != CreditsThen.getCustomerServiceID() && msg.userID != CreditsThen.getXiaoMiID() else {
        guard msg.userID != CreditsThen.tingId(), msg.userID != CreditsThen.rowId() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = AdvocatePatentThen.kindBirthType(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(m_riteUrl.prefix(6)) + kModeStr.replacingOccurrences(of: "transform", with: "pe"))] as? String == (String(mainSometimeValue) + String(kDepthMsg)) {
            //: return
            return
        }

        // 2. V2TIMMessage => ModelThen
        ...
        //: if let userInfo: MerelyMeasurable = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: MerelyMeasurable = DecisionMakerReactiveCompatible.clout(targetId: model.targetId) {
    ...
    private func alongsideOf() {
        ...
        let notifView = ReadingThen(frame: .zero)
        //: notifView.show(firstModel)
        notifView.pingShow(firstModel)
    ...
    private func keepOut() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = springEquinox() else { return false }
        //: if currentVC.isKind(of: TalkingVideoChatViewController.self) ||
        if currentVC.isKind(of: ExtentRecognizerDelegate.self) ||
            //: currentVC.isKind(of: TalkingVoiceChatViewController.self) {
            currentVC.isKind(of: FuturismViewDelegate.self)
        {
            //: return true
            return true
        }
