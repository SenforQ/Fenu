...
fileprivate let userBottomFailAutomaticStr:[Character] = ["N","e","e","d"," ","a"," ","g","i","f","t"," "]
fileprivate let notiHumanId:String = "to untower freedom bottom main word"

/*: "init(coder:) has not been implemented" :*/
fileprivate let mainMakeText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "intimatephoto_btn_no" :*/
fileprivate let k_everybodyName:String = "intiattractive"
fileprivate let main_elementMessage:String = "oto_create picture month astern"
fileprivate let noti_missBindTitle:[Character] = ["o"]

/*: "Burn After Reading" :*/
fileprivate let kNoStr:[Character] = ["B","u","r","n"," ","A","f","t","e"]
fileprivate let mainLeadingText:String = "star appeal shadow life trailr R"
fileprivate let app_statusMessage:String = "eadinthrough"

/*: "Use Photo" :*/
fileprivate let mainAddedMessage:[Character] = ["U","s","e"," ","P","h","o","t","o"]

/*: "Use Video" :*/
fileprivate let notiHesitateData:[Character] = ["U","s","e"," ","V"]
fileprivate let dream_providerData:String = "iscripteo"

/*: "intimatephoto_btn_chose" :*/
fileprivate let dreamExIdent:String = "pullntpullma"
fileprivate let notiDominantName:String = "TEP"
fileprivate let app_extentPlusTitle:String = "hoto_remaining edition first valid"

/*: "%@ (%@x1  %@ Gold coins)" :*/
fileprivate let mainPauseMsg:String = "%@don("
fileprivate let k_dataMessage:String = "  %@ "
fileprivate let showDestroyCuteFormat:String = "Gold cus start girl nor multiple"

/*: "#7E6AFF" :*/
fileprivate let m_mergeContent:String = "simultaneously feed problem builder#7E6AFF"
...
class MultiExpectView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainMakeText.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var burnAfterReadingImgV: UIImageView = {
        ...
        let img = UIImageView(image: UIImage.scribeNumber(name: (k_everybodyName.replacingOccurrences(of: "attractive", with: "m") + "ateph" + String(main_elementMessage.prefix(4)) + "btn_n" + String(noti_missBindTitle))))
    ...
    private lazy var burnAfterReadingLab: UILabel = {
        ...
        lab.text = (String(kNoStr) + String(mainLeadingText.suffix(3)) + app_statusMessage.replacingOccurrences(of: "through", with: "g")).localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var burnAfterReadingBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(shadiness), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var needGiftBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(stageLight), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var useBtn: UIButton = {
        ...
        btn.setTitle(self.type == .intimatePhoto ? (String(mainAddedMessage)).localized : (String(notiHesitateData) + dream_providerData.replacingOccurrences(of: "script", with: "d")).localized, for: .normal)
    ...
    lazy var giftView: PubliclyViewDelegate = {
...
extension MultiExpectView {
    ...
    @objc private func shadiness() {
        ...
        let imageName = burnAfterReadingBtn.isSelected ? (dreamExIdent.replacingOccurrences(of: "pull", with: "i") + notiDominantName.lowercased() + String(app_extentPlusTitle.prefix(5)) + "btn_chose") : (k_everybodyName.replacingOccurrences(of: "attractive", with: "m") + "ateph" + String(main_elementMessage.prefix(4)) + "btn_n" + String(noti_missBindTitle))
        //: burnAfterReadingImgV.image = UIImage.BundleImageNamed(name: imageName)
        burnAfterReadingImgV.image = UIImage.scribeNumber(name: imageName)
    }
}
...
extension MultiExpectView {
    ...
    @objc private func stageLight() {
        ...
        guard needGiftBtn.isSelected == false else { // 还原状态
            ...
            needGiftImgeV.image = UIImage.scribeNumber(name: (k_everybodyName.replacingOccurrences(of: "attractive", with: "m") + "ateph" + String(main_elementMessage.prefix(4)) + "btn_n" + String(noti_missBindTitle)))
        ...
        let type = (self.type == .intimatePhoto ? AdviceExpressionConvertible.intimatePhoto : AdviceExpressionConvertible.intimateVideo)
        //: TalkingChatGiftManager.share.func__sendGiftEvent(type: type) {
        FlowEditionGiftManager.share.prefaceCompletion(type: type) {
    ...
    private func pick() {
        ...
        giftView.okActionBlock = { [weak self] giftModel in
            ...
            self.needGiftImgeV.image = UIImage.scribeNumber(name: (dreamExIdent.replacingOccurrences(of: "pull", with: "i") + notiDominantName.lowercased() + String(app_extentPlusTitle.prefix(5)) + "btn_chose"))
            ...
            let giftAttrStr = NSMutableAttributedString(string: (mainPauseMsg.replacingOccurrences(of: "don", with: " ") + "%@x1" + k_dataMessage.capitalized + String(showDestroyCuteFormat.prefix(6)) + "oins)").innerArguments(self.normalGiftText, giftModel.name, giftModel.price))
            ...
            let attributes = [.foregroundColor: UIColor(hex: (String(m_mergeContent.suffix(7))))!, .font: UIFont.afterDisable(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
...
extension MultiExpectView {
    ...
    private func grind() {
        ...
        whiteBgV.addSubview(burnAfterReadingBtn)
        //: if AdministratorThen.share.loginUserMode.status != 1 &&
        if AdministratorThen.share.loginUserMode.status != 1,
    ...
    private func playEarly() {
        ...
        useBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalToSuperview().offset(-20 - main_showUrl)
