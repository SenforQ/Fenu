...
fileprivate let dreamTranslationId:[UInt8] = [0x4e,0x49,0x4e,0x53,0xf,0x44,0x48,0x43,0x42,0x55,0x1d,0xe,0x7,0x4f,0x46,0x54,0x7,0x49,0x48,0x53,0x7,0x45,0x42,0x42,0x49,0x7,0x4e,0x4a,0x57,0x4b,0x42,0x4a,0x42,0x49,0x53,0x42,0x43]

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let mainAddStr:String = "heart accept tie distinguish exitbtnJQ"
fileprivate let m_largeSweetFormat:String = "observe giving calculateetihw_"
fileprivate let userStatMessage:String = "slim copy portion vine min_shadow"

/*: "Edit" :*/
fileprivate let main_representativeTitle:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let dreamOppositeData:String = "Finishweekly can member bronze soil"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let user_miniReadFormat:String = "origin commercial officialbtnYD"
fileprivate let main_interactionId:String = "oedsupportive"

/*: "Cancel" :*/
fileprivate let appIgnoreThirdAgoIdent:String = "translation capable burn final trailCance"
fileprivate let main_allianceTitle:[Character] = ["l"]

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let dream_plotMsg:String = "strong require his controlbgQk"
fileprivate let showFourKey:String = "pop gallery soediv_"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let mAmidTransitionIdent:[UInt8] = [0x58,0x64,0x69,0x2c,0x7a,0x65,0x68,0x69,0x63,0x2c,0x60,0x69,0x62,0x6b,0x78,0x64,0x2c,0x7f,0x64,0x63,0x79,0x60,0x68,0x2c,0x6e,0x69,0x2c,0x29,0x4c,0x21,0x29,0x4c,0x2c,0x7f,0x69,0x6f,0x63,0x62,0x68,0x7f,0x22,0x2c,0x5c,0x60,0x69,0x6d,0x7f,0x69,0x2c,0x65,0x62,0x78,0x69,0x7e,0x6f,0x69,0x7c,0x78,0x2c,0x65,0x78]
...
typealias VideoEditBlock = (_ videoInfo: MechanismMeasurable?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let appMeName: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class PerusalThen: FlexibleViewController {
    ...
    private var slider: TelecastingViewDelegate?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = MechanismMeasurable() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = VisitorEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.isFind == true {
        if AdviceLandscapeReactiveCompatible.shared.isFind == true { // 正在音视频通话，静音播放
    ...
    init(style: VisitorEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (noti_senseContent - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dreamTranslationId.map{$0^39}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.scribeNumber(name: (String(mainAddStr.suffix(5)) + "bEAB_" + String(m_largeSweetFormat.suffix(6)) + "kcab" + String(userStatMessage.suffix(7)))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = PrintingReactiveCompatible.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(backEye), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(main_representativeTitle)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.fogColor(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.soil(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = PrintingReactiveCompatible.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(dreamOppositeData.prefix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.imitationClotheStatement(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.fogColor()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(contactLock), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.scribeNumber(name: (String(user_miniReadFormat.suffix(5)) + "o2AS_" + main_interactionId.replacingOccurrences(of: "supportive", with: "i") + "v_play")), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(whole(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(appIgnoreThirdAgoIdent.suffix(5)) + String(main_allianceTitle)).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(proposal), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.scribeNumber(name: (String(dream_plotMsg.suffix(4)) + "RTbF_" + String(showFourKey.suffix(6)) + "toast")))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: mAmidTransitionIdent.map{$0^12}, encoding: .utf8)!.innerArguments(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
        //: lab.font = UIFont.pingfangRugularFont(fontSize: 13)
        lab.font = UIFont.soil(fontSize: 13)
        //: v.addSubview(lab)
        v.addSubview(lab)
    ...
    lazy var useVideoView: MultiExpectView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = MultiExpectView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension PerusalThen {
    ...
    @objc func backEye() {
        ...
        ManagerRoundingPinpointTvReactiveCompatible.shared.appear()
    ...
    private func quiet(animated _: Bool = true) {
    ...
    @objc func spreadAction() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: noti_senseContent - 2 * 26, height: appMineMessage - 46 - 127 - main_showUrl)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = noti_senseContent / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: appMineMessage, width: noti_senseContent, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = TelecastingViewDelegate(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: appMeName)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - main_showUrl)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: GageSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func contactLock() {
        ...
        if self.videoEditHandler != nil {
            ...
            ManagerRoundingPinpointTvReactiveCompatible.shared.my(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.quiet()
    ...
    @objc func whole(sender: UIButton) {
...
extension PerusalThen {
    ...
    @objc private func getStartedEvaluate() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension PerusalThen {
    ...
    private func grandmaster() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(instantiate), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.exitComplete(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func titleTag() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + main_showUrl)
        }
    ...
    private func duringSumeraction() {
        /// 私密视频事件
        //: if self.currStyle == .intimateVideo {
        if self.currStyle == .intimateVideo {
    ...
    private func combineMiss() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(getStartedEvaluate),
