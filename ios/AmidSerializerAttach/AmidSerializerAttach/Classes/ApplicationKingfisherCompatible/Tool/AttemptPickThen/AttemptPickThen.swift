...
fileprivate let appInformationUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "FFD855" :*/
fileprivate let main_ratingHypothesisKey:String = "birth"
fileprivate let dream_ownerName:String = "FD8location"
...
class AttemptPickThen: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - mainLengthStr
                ...
                self.adjutant(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appInformationUrl.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (main_ratingHypothesisKey.replacingOccurrences(of: "birth", with: "F") + dream_ownerName.replacingOccurrences(of: "location", with: "55")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: userAccuracyText, y: 0, width: mainLengthStr, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - mainLengthStr, y: 0, width: mainLengthStr, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.fruitThumb(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: userAccuracyText, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.fruitThumb(0, 0, 0, 0.6)
...
extension AttemptPickThen {
    ...
    func adjutant(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - mainLengthStr,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - mainLengthStr)
    }
}
...
extension AttemptPickThen {
    ...
    @objc func enclosure(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.adjutant(startX: videoStartX)
    ...
    @objc func areaQuick(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension AttemptPickThen {
    ...
    private func channelAcross() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(enclosure(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(areaQuick(gesture:)))
    ...
    func exitComplete(_ xposition: CGFloat) {
