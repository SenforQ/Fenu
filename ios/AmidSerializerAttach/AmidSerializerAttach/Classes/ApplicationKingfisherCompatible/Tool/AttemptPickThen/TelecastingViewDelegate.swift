...
fileprivate let mainDefenseJoinIdent:[UInt8] = [0xf9,0xfe,0xf9,0xe4,0xb8,0xf3,0xff,0xf4,0xf5,0xe2,0xaa,0xb9,0xb0,0xf8,0xf1,0xe3,0xb0,0xfe,0xff,0xe4,0xb0,0xf2,0xf5,0xf5,0xfe,0xb0,0xf9,0xfd,0xe0,0xfc,0xf5,0xfd,0xf5,0xfe,0xe4,0xf5,0xf4]
...
typealias SlidingHandler = (_ direction: GageSlideDirection) -> Void
...
class TelecastingViewDelegate: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (userAccuracyText - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainDefenseJoinIdent.map{$0^144}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.versify(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: AttemptPickThen = {
        ...
        let overlay = AttemptPickThen(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension TelecastingViewDelegate {
    /// 重置位置
    //: func resetPostion() {
    func erstwhile() {
    ...
    private func versify(point: CGFloat) -> UIImage {
...
extension TelecastingViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = userAccuracyText - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.wage(isShow: false)
    }
...
extension TelecastingViewDelegate {
    ...
    private func followingActual() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(userAccuracyText)
