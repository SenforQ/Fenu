...
/*: "Finish" :*/
fileprivate let noti_remoteAnotherName:String = "with rank intro documentFinish"

/*: "Cancel" :*/
fileprivate let showInputFlameUrl:String = "install class everybody unless transferCance"
fileprivate let appPortionTrimTitle:String = "response"

/*: "btn_photo_choice_pre" :*/
fileprivate let main_analyzeStr:[Character] = ["b","t","n","_","p"]
fileprivate let k_lookUrl:String = "scan bridgehoto_ch"
fileprivate let appEstablishFormat:[Character] = ["o","i","c","e","_","p","r","e"]

/*: "btn_photo_add_nor" :*/
fileprivate let m_dismissValue:String = "btn_pout retired"
fileprivate let appChannelKey:String = "_add_norlower process"

/*: "/tmp" :*/
fileprivate let mMaleId:[Character] = ["/","t","m","p"]

/*: "Export session failed" :*/
fileprivate let mLayPath:String = "Exporcorner you appearance selection"
fileprivate let dream_outerStr:String = "ion frandom origin root pattern"
fileprivate let userGestureDestinationId:String = "ailchoiced"

/*: "Export canceled" :*/
fileprivate let userInsideIdent:[Character] = ["E","x","p","o","r","t"," ","c","a","n","c","e","l","e"]
fileprivate let m_ceaseUrl:String = "currency"

/*: "Successful!" :*/
fileprivate let userGoPath:String = "Succhave keep"
fileprivate let dream_zoneValue:String = "ul!large mechanic"

/*: "Unsupported video formats" :*/
fileprivate let m_procedureRetId:[Character] = ["U","n","s","u","p","p","o","r","t","e","d"," ","v","i","d","e","o"," "]
fileprivate let app_decentMessage:String = "wirermats"

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let mForeheadData:[Character] = ["y","y"]
fileprivate let show_themMsg:String = "yy-Mwrap without destination"
fileprivate let user_recentPath:String = "H:mmstraight system eon ready"

/*: "/tmp/video- :*/
fileprivate let kCreateerMessage:[Character] = ["/","t","m","p","/","v","i","d","e","o","-"]

/*: .mp4" :*/
fileprivate let showSeeId:String = ".mp4execute habit"
...
class GagePickTool: NSObject {
    ...
    class func examine(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = noti_senseContent * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = noti_senseContent
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (appMineMessage - height) / 2, width: noti_senseContent, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(noti_remoteAnotherName.suffix(6))).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(showInputFlameUrl.suffix(5)) + appPortionTrimTitle.replacingOccurrences(of: "response", with: "l")).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.afterDisable(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.portion()
        ...
        imagePicker?.naviTitleFont = UIFont.afterDisable(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.manualColor()
        ...
        imagePicker?.iconThemeColor = UIColor.fogColor()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.scribeNumber(name: (String(main_analyzeStr) + String(k_lookUrl.suffix(7)) + String(appEstablishFormat)))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.scribeNumber(name: (String(m_dismissValue.prefix(5)) + "hoto" + String(appChannelKey.prefix(8))))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.afterDisable(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(noti_remoteAnotherName.suffix(6))).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(noti_remoteAnotherName.suffix(6))).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.fogColor()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: noti_senseContent - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func spreadInput(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(mMaleId)))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(mMaleId))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(mLayPath.prefix(5)) + "t sess" + String(dream_outerStr.prefix(5)) + userGestureDestinationId.replacingOccurrences(of: "choice", with: "e")))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(userInsideIdent) + m_ceaseUrl.replacingOccurrences(of: "currency", with: "d")))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(userGoPath.prefix(4)) + "essf" + String(dream_zoneValue.prefix(3))))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(m_procedureRetId) + app_decentMessage.replacingOccurrences(of: "wire", with: "fo")))
            }
        }
    }
    ...
    private class func viaTitle() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(mForeheadData) + String(show_themMsg.prefix(4)) + "M-dd-H" + String(user_recentPath.prefix(4)) + ":ss-SSS")
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(kCreateerMessage)) + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(showSeeId.prefix(4))))
        //: return outputPath
        return outputPath
    }
}
