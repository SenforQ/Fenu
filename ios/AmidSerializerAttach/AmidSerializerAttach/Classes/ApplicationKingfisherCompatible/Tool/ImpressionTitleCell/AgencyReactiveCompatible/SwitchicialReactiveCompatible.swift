...
fileprivate let m_exactlyReaderUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
// __CLOSE_PRINT__
//
//  SwitchicialReactiveCompatible.swift
//  AbroadTalking
//
...
class SwitchicialReactiveCompatible: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        self.setupCombineRestriction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: m_exactlyReaderUrl.reversed(), encoding: .utf8)!)
    }
...
extension SwitchicialReactiveCompatible {
    //: func setCellModel(model: STFilterLooksModel) {
    func compareMic(model: OnReactiveCompatible) {
        //: self.iconView.image = UIImage.BundleImageNamed(name: model.filterNormalImageName)
        self.iconView.image = UIImage.scribeNumber(name: model.filterNormalImageName)
        ...
        if model.isSelected {
            ...
            self.nameLab.textColor = UIColor.fogColor()
            //: }else {
        } else {
