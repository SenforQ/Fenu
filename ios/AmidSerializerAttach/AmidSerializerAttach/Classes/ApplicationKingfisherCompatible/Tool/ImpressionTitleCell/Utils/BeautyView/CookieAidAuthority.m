...
@interface CustomerShData : NSObject

+ (instancetype)sharedInstance;

//: 请打开相机权限
@property (nonatomic, copy) NSString *mainFromMsg;

//: Width
@property (nonatomic, copy) NSString *main_eyeContent;

//: Height
@property (nonatomic, copy) NSString *user_reValue;

@end

@implementation CustomerShData

+ (instancetype)sharedInstance {
    static CustomerShData *instance = nil;
...
+ (NSData *)CustomerShDataToData:(NSArray<NSNumber *> *)value {
...
- (Byte *)CustomerShDataToCache:(Byte *)data {
    int gen = data[0];
    Byte baileyBridge = data[1];
    int valleyReveal = data[2];
    for (int i = valleyReveal; i < valleyReveal + gen; i++) {
        int value = data[i] + baileyBridge;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[valleyReveal + gen] = 0;
    return data + valleyReveal;
}

- (NSString *)StringFromCustomerShData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self CustomerShDataToCache:data]];
}

//: Width
- (NSString *)main_eyeContent {
    if (!_main_eyeContent) {
		NSArray<NSNumber *> *origin = @[@5, @72, @5, @63, @231, @15, @33, @28, @44, @32, @20];
		NSData *data = [CustomerShData CustomerShDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _main_eyeContent = [self StringFromCustomerShData:value];
    }
    return _main_eyeContent;
}

//: Height
- (NSString *)user_reValue {
    if (!_user_reValue) {
		NSArray<NSNumber *> *origin = @[@6, @39, @11, @239, @42, @12, @96, @89, @241, @28, @70, @33, @62, @66, @64, @65, @77, @44];
		NSData *data = [CustomerShData CustomerShDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _user_reValue = [self StringFromCustomerShData:value];
    }
    return _user_reValue;
}

//: 请打开相机权限
- (NSString *)mainFromMsg {
    if (!_mainFromMsg) {
		NSArray<NSNumber *> *origin = @[@21, @52, @12, @128, @150, @30, @115, @230, @130, @152, @115, @109, @180, @123, @131, @178, @85, @95, @177, @136, @76, @179, @103, @132, @178, @104, @134, @178, @105, @79, @181, @101, @92, @110];
		NSData *data = [CustomerShData CustomerShDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _mainFromMsg = [self StringFromCustomerShData:value];
    }
    return _mainFromMsg;
}
...
@interface CookieAidAuthority () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@implementation CookieAidAuthority

//: - (instancetype)initWithDevicePosition:(AVCaptureDevicePosition)iDevicePosition
- (instancetype)initWithNarrow:(AVCaptureDevicePosition)iDevicePosition
                        //: sessionPresset:(AVCaptureSessionPreset)sessionPreset
                        uniform:(AVCaptureSessionPreset)sessionPreset
                                   //: fps:(int)iFPS
                                   yieldOutput:(int)iFPS
                         //: needYuvOutput:(BOOL)needYuvOutput
                         angle:(BOOL)needYuvOutput
{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
        ...
        self.videoDevice = [self byArray:iDevicePosition];
...
- (void)video:(CGPoint)point whiteMain:(CGRect)frame
{
    ...
    [self process:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           tableOfTitles:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                lifestyle:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)process:(AVCaptureFocusMode)focusMode tableOfTitles:(AVCaptureExposureMode)exposureMode lifestyle:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self program:^(AVCaptureDevice *captureDevice) {
...
- (void)program:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self byArray:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self without]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:[CustomerShData sharedInstance].main_eyeContent] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:[CustomerShData sharedInstance].user_reValue] floatValue];
...
- (CGRect)originInstance:(CGRect)rect format:(BOOL)bScaleToFit
{
    //: CGRect rectRet = rect;
    CGRect rectRet = rect;
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:[CustomerShData sharedInstance].main_eyeContent] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:[CustomerShData sharedInstance].user_reValue] floatValue];
...
- (BOOL)without
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:[CustomerShData sharedInstance].mainFromMsg
...
- (AVCaptureDevice *)byArray:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(quote_strong:powderFlask:infuse:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate quote_strong:captureOutput powderFlask:sampleBuffer infuse:connection];
        }
    }
