...
protocol TowardNavigationDelegate: AnyObject {
...
class MerelyViewDelegate: UIViewController {
    ...
    weak var delegate: TowardNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = CreditsThen.getScreenHeight() - (topDistance + kTitle_H)
        var height = CreditsThen.valueHeight() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= dream_displayUrl
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: CreditsThen.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: CreditsThen.drawSearchion(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(CreditsThen.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(CreditsThen.drawSearchion()), height: 0)
        ...
        scrollView.semanticContentAttribute = PrintingReactiveCompatible.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: CreditsThen.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: CreditsThen.drawSearchion()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: CreditsThen.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: CreditsThen.drawSearchion()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = PrintingReactiveCompatible.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(beak), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = CreditsThen.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = CreditsThen.drawSearchion() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(CreditsThen.getScreenWidth()-originX-titleWidth)
            let x = PrintingReactiveCompatible.shared.direction == .leftToRight ? originX:(CreditsThen.drawSearchion()-originX-titleWidth)
...
extension MerelyViewDelegate {
    ...
    @objc private func beak(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        elite(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * CreditsThen.getScreenWidth()
        let offset_x = CGFloat(index) * CreditsThen.drawSearchion()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        negotiation(withIndex: index)
        //: seletedViewController(withIndex: index)
        seleted(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func negotiation(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * CreditsThen.getScreenWidth()
        let origin_x = CGFloat(index) * CreditsThen.drawSearchion()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: CreditsThen.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: CreditsThen.drawSearchion(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func elite(_ button: UIButton) {
    ...
    func attorn(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= CreditsThen.getScreenWidth() {
        if titleScrollView.contentSize.width <= CreditsThen.drawSearchion() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - CreditsThen.getScreenWidth() * 0.5
        var offset_x = sender.center.x - CreditsThen.drawSearchion() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - CreditsThen.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - CreditsThen.drawSearchion()
...
extension MerelyViewDelegate {
    ...
    func seleted(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.squareElect(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension MerelyViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / CreditsThen.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / CreditsThen.drawSearchion())
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / CreditsThen.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / CreditsThen.drawSearchion())
        ...
        //: let scaleR = scrollView.contentOffset.x / CreditsThen.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / CreditsThen.drawSearchion() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
