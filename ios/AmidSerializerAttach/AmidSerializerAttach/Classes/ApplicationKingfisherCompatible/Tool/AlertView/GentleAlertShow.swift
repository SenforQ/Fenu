...
fileprivate let showForwardValueTitle:String = "btn_shprocessor sleeve clothes today"
fileprivate let k_goMessage:String = "icon failure stat eonow_a"
fileprivate let show_withinTitle:[Character] = ["l","e","r","t","B","a","c","k","g","r","o","u","n","d"]

/*: "btn_alertBackground_false" :*/
fileprivate let m_particleStr:String = "protocol time bucketbtn_aler"
fileprivate let dream_tireName:[Character] = ["t","B","a","c","k","g","r","o","u","n","d","_","f","a","l","s","e"]
...
public class GentleAlertShow: NSObject {
    /// 通用回调
...
public extension GentleAlertShow {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: GainGageAlertConfig) -> Void
    ...
    internal class func aboveSixth(title: String? = nil,
                                   //: message: String?  = nil,
                                   message: String? = nil,
                                   ...
                                   config: GainGageAlertConfig? = nil)
    {
        //: customAlert(title: title,
        begNativeConfig(title: title,
                        //: message: message,
                        message: message,
    ...
    @objc internal class func behindAlert(title: String? = nil,
    ...
    class func begNativeConfig(title: String? = nil,
                               ...
                               config: GainGageAlertConfig? = nil)
    {
        ...
        let alertView = WarnAlertView(title: title,
        ...
        //: CreditsThen.getWindow().addSubview(alertView)
        CreditsThen.captain().addSubview(alertView)
    ...
    class func someButton() {
        //: CreditsThen.getWindow().subviews.forEach { (view) in
        CreditsThen.captain().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: WarnAlertView.self) {
    ...
    class func theWrite(title: String, isTouch: Bool) {
        //: CreditsThen.getWindow().subviews.forEach { (view) in
        CreditsThen.captain().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: WarnAlertView.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: WarnAlertView = view as! WarnAlertView
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.alongGold(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class GainGageAlertConfig {
    /// 执行动画时间
    ...
    public var maskType: MerelyMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .afterDisable(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.manualColor()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .afterDisable(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.manualColor()
    ...
    public var buttonFont: UIFont = .afterDisable(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.fogColor()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(showForwardValueTitle.prefix(6)) + String(k_goMessage.suffix(4)) + String(show_withinTitle))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(m_particleStr.suffix(8)) + String(dream_tireName))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .afterDisable(type: .Medium, fontSize: 16)
