...
fileprivate let user_topMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
let noti_contentData = 80.0
//: let kProgressHUD_cornerRadius = 14.0
let k_storageId = 14.0
//: let kProgressHUD_alpha        = 0.9
let mFormatProductionContent = 0.9
//: let kBackgroundView_alpha     = 0.6
let app_eventStr = 0.6
//: let kAnimationInterval        = 0.2
let dream_locationText = 0.2
//: let kTransformScale           = 0.9
let m_lengthText = 0.9

//: open class ProgressHUD: UIView {
open class VillageView: UIView {
    //: required public init?(coder: NSCoder) {
    public required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: user_topMsg.reversed(), encoding: .utf8)!)
    }

    //: static var shared = ProgressHUD()
    static var shared = VillageView()
    ...
    //: class func show() {
    class func projectShow() {
        //: show(superView: nil)
        feeView(superView: nil)
    }

    //: class func show(superView: UIView?) {
    class func feeView(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                VillageView.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                VillageView.shared.activityIndicator.center = VillageView.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(VillageView.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                VillageView.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                VillageView.shared.activityIndicator.center = VillageView.shared.center
                //: CreditsThen.getWindow().addSubview(ProgressHUD.shared)
                CreditsThen.captain().addSubview(VillageView.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        VillageView.shared.gageAnimating()
    }

    //: class func dismiss() {
    class func statusDismiss() {
        //: ProgressHUD.shared.hud_stopAnimating()
        VillageView.shared.noticeAnimating()
    }

    //: private func hud_startAnimating() {
    private func gageAnimating() {
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: self.backgroundColor = UIColor(white: 0, alpha: 0)
            self.backgroundColor = UIColor(white: 0, alpha: 0)
            //: self.activityIndicator.transform = CGAffineTransform(scaleX: kTransformScale, y: kTransformScale)
            self.activityIndicator.transform = CGAffineTransform(scaleX: m_lengthText, y: m_lengthText)
            //: self.activityIndicator.alpha = 0
            self.activityIndicator.alpha = 0
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: dream_locationText) {
                //: self.backgroundColor = UIColor(white: 0, alpha: kBackgroundView_alpha)
                self.backgroundColor = UIColor(white: 0, alpha: app_eventStr)
                //: self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                //: self.activityIndicator.alpha = kProgressHUD_alpha
                self.activityIndicator.alpha = mFormatProductionContent
    ...
    private func noticeAnimating() {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: dream_locationText) {
                ...
                VillageView.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: noti_contentData, height: noti_contentData)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = k_storageId
...
extension VillageView {
    //: class func toast(_ str: String?) {
    class func asBalance(_ str: String?) {
        //: toast(str, showTime: 1)
        fragment(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func fragment(_ str: String?, showTime: CGFloat) {
        ...
        titleLab.textColor = .white
        //: CreditsThen.getWindow().addSubview(titleLab)
        CreditsThen.captain().addSubview(titleLab)
        //: let size = titleLab.sizeThatFits(CGSize(width: UIScreen.main.bounds.width - 40, height: CGFloat(MAXFLOAT)))
        let size = titleLab.sizeThatFits(CGSize(width: UIScreen.main.bounds.width - 40, height: CGFloat(MAXFLOAT)))
        //: titleLab.center = CreditsThen.getWindow().center
        titleLab.center = CreditsThen.captain().center
