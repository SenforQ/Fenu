...
/*: "Select a date" :*/
fileprivate let showIonValue:[Character] = ["S","e","l","e","c","t"," ","a"]
fileprivate let showSweepIdent:String = "aide replacement pretty date"
...
protocol BeautyObjectProtocol: NSObjectProtocol {
...
class QuickDateView: ConfirmPickView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: BeautyObjectProtocol?
    ...
    override func eigenvalueOfAMatrix() {
        ...
        titleString = (String(showIonValue) + String(showSweepIdent.suffix(5))).localized
        ...
        howeverDay(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func howeverDay(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func prove(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.inmate(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.inmate(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return ofAllTime(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func ofAllTime(year: Int, month: Int) -> Int {
