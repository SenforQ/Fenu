...
/*: "RSA加密失败，str:  :*/
fileprivate let app_powderOncePanelUrl:[Character] = ["R","S","A","\u{52a0}","密","失","败","，","s"]
fileprivate let notiFrontUrl:[Character] = ["t","r",":"," "]

/*: "weixin://" :*/
fileprivate let notiSupposedMessage:String = "weixin:accounting pitch blank capture never"
fileprivate let k_sufficientValue:String = "//"

/*: "dingtalk://" :*/
fileprivate let userAdministrativeMsg:[Character] = ["d","i","n","g","t","a","l","k"]
fileprivate let userThreadMsg:String = "professional//"

/*: "lark://" :*/
fileprivate let noti_providerKey:[Character] = ["l","a","r","k",":"]
fileprivate let appHistoryFiscalFormat:String = "//"

/*: "wxwork://" :*/
fileprivate let mainSteerUrl:[Character] = ["w","x","w","o"]
fileprivate let mainTowardKey:String = "rk://which properly deal opposition"

/*: "deviceId" :*/
fileprivate let app_channelStr:[UInt8] = [0xf7,0xf6,0xe5,0xfa,0xf0,0xf6,0xda,0xf7]

/*: "weixin" :*/
fileprivate let kBodyIdent:String = "weihundredi"
fileprivate let mainNeverPath:[Character] = ["n"]

/*: "wxwork" :*/
fileprivate let user_panColumnIdent:[Character] = ["w","x","w","o","r","k"]

/*: "dingtalk" :*/
fileprivate let kRangeFormalData:String = "dinenvelope"

/*: "lark" :*/
fileprivate let show_moveId:String = "lbasicrk"
...
extension UIDevice {
    ...
    class func artifactGuidance(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.menuNumbereractionId())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
    ...
    class func menuNumbereractionId() -> String {
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func shouldStr(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (String(app_powderOncePanelUrl) + String(notiFrontUrl)) + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (String(notiSupposedMessage.prefix(7)) + k_sufficientValue.capitalized)
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (String(userAdministrativeMsg) + userThreadMsg.replacingOccurrences(of: "professional", with: ":"))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(noti_providerKey) + appHistoryFiscalFormat.capitalized)
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (String(mainSteerUrl) + String(mainTowardKey.prefix(5)))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(show_agentAvailableFormat) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: show_agentAvailableFormat)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = BubbleVisitorReactiveCompatible.scribeProxy() || BubbleVisitorReactiveCompatible.toolRequest()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = shouldStr(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: app_channelStr.map{$0^147}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((kBodyIdent.replacingOccurrences(of: "hundred", with: "x") + String(mainNeverPath)))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((String(user_panColumnIdent)))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((kRangeFormalData.replacingOccurrences(of: "envelope", with: "g") + "talk"))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((show_moveId.replacingOccurrences(of: "basic", with: "a")))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
