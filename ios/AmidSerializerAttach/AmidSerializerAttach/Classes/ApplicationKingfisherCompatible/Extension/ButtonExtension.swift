...
/*: "recharge" :*/
fileprivate let kMightName:String = "RECHAR"
fileprivate let appCalculationTitle:[Character] = ["g","e"]

/*: "recharge_ :*/
fileprivate let show_allianceMsg:String = "recharconcernede"
fileprivate let app_greenText:String = "_"

/*: ".png" :*/
fileprivate let notiEditorId:[Character] = [".","p","n","g"]

/*: .png" :*/
fileprivate let main_outcomeText:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let user_activeButtonPersonIdent:String = "huge look asset premium.jpg"

/*: .jpg" :*/
fileprivate let kChildIdent:String = ".jpgmutual exclusive proposal due"
...
class WordOfAdviceThen: UIButton {
    ...
    var imageAlignment: RecordSumerval = .left
...
extension UIButton {
    ...
    func witch(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        refresh(interval: interval, topOffset: 0)
    }
    ...
    func refresh(interval: CGFloat, topOffset: CGFloat) {
    ...
    func localPrice(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func toastrackUpwardlyState(color: UIColor, forState: UIControl.State) {
    ...
    func arm(urlStr: String,
             //: placeImg: UIImage? = UIImage.placeImgSquare(),
             placeImg: UIImage? = UIImage.partnerPlan(),
             ...
             loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: kKeyInsertValue.generalGame(), with: dreamVoiceMarginStr)
    ...
    func amTotaleraction(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((kMightName.lowercased() + String(appCalculationTitle))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (kMightName.lowercased() + String(appCalculationTitle)), with: (show_allianceMsg.replacingOccurrences(of: "concerned", with: "g") + app_greenText.capitalized) + "\(dreamStreamKey)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.arm(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(notiEditorId))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(notiEditorId)), with: "_\(dreamStreamKey)" + (String(main_outcomeText)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(user_activeButtonPersonIdent.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(user_activeButtonPersonIdent.suffix(4))), with: "_\(dreamStreamKey)" + (String(kChildIdent.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.arm(urlStr: newUrl, placeImg: nil)
    }
