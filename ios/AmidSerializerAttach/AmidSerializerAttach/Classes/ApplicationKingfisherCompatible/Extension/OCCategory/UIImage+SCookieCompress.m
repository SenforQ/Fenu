...
- (UIImage *)manager {
    ...
    return [UIImage imageWithData:[self compressedFromAssemblage]];
}
...
- (UIImage *)send:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self createAcrossSize:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self resized:size];
...
- (CGSize)createAcrossSize:(Boolean)isSession {
...
+ (NSData *)handle:(UIImage *)image stillMore:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG value:scalesize view:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG limit:(fileSize*1024)];
    }
...
- (UIImage*)shade:(CGSize)targetSize{
...
- (UIImage *)value:(CGSize)targetSize view:(BOOL)isScale{
...
- (NSData *)limit:(NSInteger)fileSize{
