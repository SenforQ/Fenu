...
/*: "http://stat. :*/
fileprivate let show_dueMsg:String = "http:/flexible note demonstrate size suite"
fileprivate let mainClearName:[Character] = ["/","s","t","a","t","."]

/*: .com" :*/
fileprivate let dreamDueTitle:String = "fresh chance rocket minimize.com"

/*: "http://" :*/
fileprivate let k_clearMsg:[Character] = ["h","t","t","p",":"]
fileprivate let kBalanceName:String = "woodwood"

/*: "https://" :*/
fileprivate let dreamThenOwnerKey:String = "hnosep"

/*: "http://m. :*/
fileprivate let dream_sheStr:[Character] = ["h","t","t","p",":","/","/","m","."]

/*: "http:// :*/
fileprivate let kUpName:String = "broad own harassment casthttp://"

/*: "3,90" :*/
fileprivate let dream_zoneStr:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let notiLayerPath:String = "1quick0"
fileprivate let notiSensorHistoryData:String = "error"

/*: "MM-dd-yyyy" :*/
fileprivate let m_successOperateMessage:[Character] = ["M","M","-","d","d","-","y","y","y"]
fileprivate let mThatBoardId:String = "confirm"
...
class DoingicialReactiveCompatible: NSObject {
    ...
    var requestType: FireRequestType?
    ...
    override init() {
        ...
        self.requestServer = BubbleVisitorReactiveCompatible.share.serverUrlStr
...
struct DetectModelType: HandyJSON {
    //: var errno: Int!
...
public struct FairlyAdaptErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct MigrationMeasurable: HandyJSON {
    ...
    var CSConfig = MarkMeasurable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(show_dueMsg.prefix(6)) + String(mainClearName)) + "\(dreamVoiceMarginStr)" + (String(dreamDueTitle.suffix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(k_clearMsg) + kBalanceName.replacingOccurrences(of: "wood", with: "/"))) && !statDomain.hasPrefix((dreamThenOwnerKey.replacingOccurrences(of: "nose", with: "tt") + "s://")) {
                //: return "http://".appending(statDomain)
                return (String(k_clearMsg) + kBalanceName.replacingOccurrences(of: "wood", with: "/")).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(dream_sheStr)) + "\(dreamVoiceMarginStr)" + (String(dreamDueTitle.suffix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(kUpName.suffix(7))) + "\(urlDomain)"
        }
    }
...
public class ApplicationMeasurable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [ImplementationInexpensivenessTransformable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [ImplementationInexpensivenessTransformable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [ImplementationInexpensivenessTransformable] = [] // 语音收费设置
    ...
    var vipSetting = [ShowBillTransformable]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [CutModelType]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = MigrationLimitModel() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class RemovalReactiveCompatible: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [MigrationHandyJSON]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [MigrationHandyJSON]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [AccountGalleryModel]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (dream_zoneStr.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.forthcoming(self.birthday ?? (notiLayerPath.replacingOccurrences(of: "quick", with: "99") + "-01-0" + notiSensorHistoryData.replacingOccurrences(of: "error", with: "1")), dateFormat: (String(m_successOperateMessage) + mThatBoardId.replacingOccurrences(of: "confirm", with: "y")))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.nigh().year else { return "0" }
...
struct AccountGalleryModel: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct MigrationHandyJSON: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func saveerTo(compareTo: MigrationHandyJSON) -> Bool {
        //: return
        return
...
struct InjuryModelType: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [MultiTransformable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [MultiTransformable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct MultiTransformable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [MigrationHandyJSON]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct MarkMeasurable: HandyJSON {
...
struct ShowBillTransformable: HandyJSON {
...
struct MigrationLimitModel: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
